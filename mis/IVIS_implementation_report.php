...
function generateForIndicator1($conn,$facility,$qry,$chkService,$monthYear,$districts,$m) {
    ...
    $tablevalues1 = '';
							...
							if($chkService!='') {
								...
								else { $c11='NV'; $bgc1='FFFFFF'; $column1[$districts][$i]=0; }
...
function generateForIndicator4($conn,$facility,$qry,$chkService,$monthYear,$districts,$m) {
    ...
    $tablevalues1 = '';
	                         if($chkService!='') {
									 $c11 = 'NA';
									 $bgc1='F9BB87';
									if (is_numeric($datast->mon1_pw_4doses)) {
									$np1 = $datast->mon1_pw_4doses;
										if($np1==0)
										{
											$bgc1='F9BB87';
											$c11 = 0;
								...
								else { $c11='NV'; $bgc1='FFFFFF'; $column4[$districts][$m]=0;  }
...
function generateForIndicator5($conn,$facility,$qry,$chkService,$monthYear,$districts,$m) {
    ...
    $tablevalues1 = '';
							 ...
							 if($chkService!='')
									...
									$c6 = 0;
									if ($datast->am_register!='' && $datast->am_register=='yes') {
										$c6 = 'Yes';
										$bgc='CCFF66';
...
function generateForIndicator6($conn,$facility,$qry,$chkService,$monthYear,$districts,$m) {
    ...
    $tablevalues1 = '';
							 ...
							 if($chkService!='')
								   {
									$c7 = 0;
									if ($datast->am_coun_mat!='' && $datast->am_coun_mat== 'yes') {
										$c7 = 'Yes';
										$bgc='CCFF66';
...
<?php               
                        	if($_REQUEST['uType']!='')
					{
						...
						$sqlAWCvisit = mysqli_query($conn, "SELECT f.*, m.* FROM ivis_facilities f LEFT JOIN ( SELECT * FROM a27ycjonhei3rt5h4badxn WHERE 1=1 $qry $qry1 GROUP BY hf_name) m ON f.center_code = m.hf_name ORDER BY f.districts");
...
<?php $i++; $j++; } } 
					//////////////////////////// Indicator Wise
						...
						$sqlAWCvisit = mysqli_query($conn, "SELECT f.*, m.* FROM ivis_facilities f LEFT JOIN ( SELECT * FROM a27ycjonhei3rt5h4badxn WHERE 1=1 $qry GROUP BY hf_name) m ON f.center_code = m.hf_name ORDER BY f.districts");
						while ($dataAWCvisit = mysqli_fetch_object($sqlAWCvisit)) { 
							 $bgc='';
							 $chkService='';
							if($dataAWCvisit->is_admin=='yes')
							...
							$tablevalues.=' <tr>
								...
								if($chkService!='')
								{
									 ...
									 $c1 = 'NA';
									 $bgc='F9BB87';
									$np = $dataAWCvisit->mon1_ref_vhsnd+$dataAWCvisit->mon1_ref_pmsma+$dataAWCvisit->mon1_ref_g_o+$dataAWCvisit->mon1_ref_oth;
								...
								<td class="td_head3" align="center" valign=bottom bgcolor="'.$bgc.'" sdval="0.5" sdnum="1033;" data-fill-color="'.$bgc.'" data-f-color="000000" data-b-r-s="thin" data-b-r-c="000000" data-b-r-s="thin" data-b-r-c="000000">';
									...
									if($chkService!='')
								    {
									if (is_numeric($dataAWCvisit->mon1_pw_4doses)) {
										$c4 = $dataAWCvisit->mon1_pw_4doses;
										$bgc='CCFF66';
										if($c4==0)
										{
											$bgc='F9BB87';
								...
								<td class="td_head3" align="center" valign=bottom bgcolor="'.$bgc.'" sdval="0.5" sdnum="1033;" data-fill-color="'.$bgc.'" data-f-color="000000" data-b-r-s="thin" data-b-r-c="000000" data-b-r-s="thin" data-b-r-c="000000">'; 
								  ...
								  $c5 = 0;
									...
									//{
									 ...
									 $c5ctot=round((($dataAWCvisit->mon1_pw_4doses*100)/$np),1);

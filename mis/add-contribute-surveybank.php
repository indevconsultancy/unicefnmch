...
define("title", "Add Tool Archive | MQUAD");
...
if (isset($_POST['submit'])) {
	...
	$survey_name = sanitizeInput($_POST['survey_name'], $conn);
	$client_name = sanitizeInput($_POST['client_name'], $conn);
	$category_id = $_POST['category_id'];
	$category_id = implode(",", $category_id);
	$description = sanitizeInput($_POST['description'], $conn);
	$tool_access = $_POST['tool_access'];
	$questionnaire_type = $_POST['questionnaire_type'];
	$from_date = $_POST['from_date'];
	$to_date = $_POST['to_date'];

	if ($questionnaire_type == "File") {
		$filename = $_FILES['uploaded_questionnaire']['name'];
		$ext = pathinfo($filename, PATHINFO_EXTENSION);
		$filename =  $filename . $client_id . "-" . date('his') . "." . $ext;
		...
		if (in_array($ext, $allowed_extensions)) {
			...
			$_SESSION['error'] = 'Invalid file format. Allowed formats: pdf, xls, xlsx, doc, docx, csv';
	...
	if ($questionnaire_type == "Link") {
		...
		if (!filter_var($_POST['questionnaire_link'], FILTER_VALIDATE_URL)) {
			$_SESSION['status_error'] = "Invalid questionnaire link";
			$_SESSION['status_error_code'] = "warning";
			exit();
		} else {
			$questionnaire_file = htmlspecialchars($_POST['questionnaire_link'], ENT_QUOTES, 'UTF-8');
		}
	}


	$tool_archive_status = ($_SESSION['role_id'] == 1) ? 1 : 0;

	// echo "INSERT INTO survey_bank SET survey_title = '$survey_name',description = '$description',category_id = '$category_id',tool_access = '$tool_access',client_name = '$client_name',uploaded_questionnaire = '$questionnaire_file',tool_study_year_to = '$to_date',tool_study_year_from = '$from_date',questionnaire_type = '$questionnaire_type',user_id='".$user_id."',tool_archive_status='$tool_archive_status'";
	// die();
			...
			<div class="col-lg-12">
				<nav aria-label="breadcrumb">
					<ol class="breadcrumb">
						...
						<li class="breadcrumb-item" aria-current="page"><i class="fa fa-plus"></i>Add Tools</li>
					</ol>
				</nav>
			</div>
		</div>
					...
					<div class="panel-body">
						<div class="form">
							<form class="form-horizontal" id="myForm" method="post" enctype="multipart/form-data">
								...
								<div class="mb-3 row">
									<label for="questionnaire_type" class="control-label col-lg-2">Tool: <span style="color:red;">*</span></label>
									<div class="col-lg-10">
										<input type="radio" id="type1" required name="questionnaire_type" value="Link">
										<label for="type1"> Link</label>
										<input style=" margin-left: 10px;" type="radio" id="type2" required name="questionnaire_type" value="File">
								...
								<div id="questionnaire_link_field" style="display:none;">
									<div class="mb-3 row">
										...
										<label for="questionnaire_link" class="control-label col-lg-2">Add Link: <span style="color:red;">*</span></label>
										<div class="col-lg-10">

...
<?php
// && $_REQUEST['process']=='publishWebFormUpload'
	
		if($_GET['survey_id']){
			
			$sql = mysqli_query($conn,"select status from survey where id = '".$_GET['survey_id']."'");
			$surveyData = mysqli_fetch_object($sql);
			$publishStatus = $surveyData->status;
		if($publishStatus == 0){
			//$delSurveyLoockups = mysqli_query($conn, "Delete from survey_loockups where survey_id = '".$_GET['survey_id']."'");
			$delNormalGroup = mysqli_query($conn, "Delete from normal_groups where survey_id = '".$_GET['survey_id']."'");
			$delQuestionsGroup = mysqli_query($conn, "Delete from questions_group where survey_id = '".$_GET['survey_id']."'");
			$delReportFormat = mysqli_query($conn, "Delete from report_format where survey_id = '".$_GET['survey_id']."'");
			$delQuestions = mysqli_query($conn, "Delete from questions where survey_id = '".$_GET['survey_id']."'");
			$delQuestionsLanguage = mysqli_query($conn, "Delete from questions_language where survey_id = '".$_GET['survey_id']."'");
			$delOptions = mysqli_query($conn, "Delete from options where survey_id = '".$_GET['survey_id']."'");
			$delOptionsLanguage = mysqli_query($conn, "Delete from options_language where survey_id = '".$_GET['survey_id']."'");
			
			set_include_path(get_include_path() . PATH_SEPARATOR . 'Classes/');
			include_once 'PHPExcel/Classes/PHPExcel/IOFactory.php';
			//$file = $tmp_name;//'/media/sf_E_DRIVE/om/newData.xlsx';
			//$filelocation = '/sampling/ques.xls';
			 $clientid=$_SESSION['client_id'];
			$clientId="C".$clientid;
			$survey_id = $_GET['survey_id'];
			$inserted_form_id = $_GET['survey_id'];
			$url = $_GET['url'];
			...
			$language_id ='1'; //$_POST['language_id'];

			$sqlsurvey="select survey_name,questinnour_file from survey where id='".$survey_id."'";
			$qrySurvey=mysqli_query($conn,$sqlsurvey);
			$dataSurvey=mysqli_fetch_array($qrySurvey);
			$questinnour_file=$dataSurvey['questinnour_file'];
			$formName=$dataSurvey['survey_name'];
		
	
			$ext = pathinfo($filename, PATHINFO_EXTENSION);
			$file_name = $formName."-".$unique_id.".".$ext;
			if($survey_id!=''){
				$insertForm = mysqli_query($conn,"UPDATE survey SET questinnour_file='".$file_name."' where id='".$survey_id."'");
				
			} 
			
			$fileURL = 'https://unicef.indevconsultancy.in/mis/uploaded_questionnaire/'.$clientId.'/'.$file_name;
			...
			/** Include path **/
								...
								if ($type != ''){
									...
									if($language_id=="1"){
										...
										*/
										...
										$insertdataa = mysqli_query($conn,$question_sql);
										if($insertdataa==false){
											...
											$insertdataa2 = mysqli_query($conn,$question_sql);
										}
										$last_id=mysqli_insert_id($conn);
										$question_id=$last_id;
									...
									if(count($quest_labelarray)>0)
									{
										...
										for($qns=0; $qns<count($quest_labelarray['language_id']); $qns++ )
										{
											...
											*/
											...
											if($questions_type_id == '11' || $questions_type_id == '8'){
												$question_name = '';
											}
				...
				$insertdatas = mysqli_query($conn,$question_sql);
				 $last_id=mysqli_insert_id($conn);
				$question_id=$last_id;
			   // $question_language_sql ="insert into questions_language set language_id='".$language_id."', question_id='".$question_id."' ,question_name='Your survey is complete. Do you want to submit?',question_description='Desc',questions_type_id='16',repeat_count='',question_input_type_id='',max_input='0',screen_no='1',input_field_type='note',read_only='',calculation='',ref_table='',group_id='0',sequence_no='".$sequence_no."',field_name='submit',validation_id='0',title='Submit',category_name='',parameters='',survey_id='".$inserted_form_id."', relevant='',constraints='',required='', constraint_msg='' , choice_filter='' , appearance='', choice_relation='',default_response='', paradata='', normal_group_id='', unique_id='', preserve='' ";
				...
				//echo "success ajax page call webv1.js";
			if($insertdataa || $insertdataa2){
				$resArr = array("status"=>1,"msg"=>"success");
				echo json_encode($resArr);
			}
			else{
				$resArr = array("status"=>0,"msg"=>"Failed_second");
				echo json_encode($resArr);
			}
		}
		else{
			$resArr = array("status"=>2,"msg"=>"Web form is already published!!");
			echo json_encode($resArr);
		}
	}else{
		//echo "survey id not found in web page";
		$resArr = array("status"=>0,"msg"=>"Failed_Last");
		echo json_encode($resArr);
	}	

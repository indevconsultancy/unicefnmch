...
if (isset($_POST['survey_name_txt'])) { 
    $survey_name_txt =SpecialCharRemove(sanitizeInput($_POST['survey_name_txt'],$conn));
...
if (isset($_POST['validate_file']) && $_POST['validate_file'] === 'validate-file') {
    if (isset($_FILES['structure']) && $_FILES['structure']['error'] === UPLOAD_ERR_OK) {
        $filename = $_FILES['structure']['name'];
        $tmp_name = $_FILES['structure']['tmp_name'];
        $errors = [];
        $errors = verifySurvey($conn, $filename, $tmp_name);

        echo json_encode($errors);
    } else {
       echo json_encode(['error' => 'File upload failed or file is missing.']);
    }
} 

?>
<?php
function remove_spaces_around_operators($input_string) {
    // Define the pattern for matching spaces around the operators
    $pattern = '/\s*(<=|>=|<>|==|!=|=|>|<|\+|\-|\*|\/|&&|\|\|&)\s*/';
    // Replace the matched spaces around the operators with the operators themselves
    $cleaned_string = preg_replace($pattern, '$1', $input_string);
    return $cleaned_string;
}
// function remove_spaces_constraint_calculation($input_string) {
    // $pattern = '/\s*(&&|&|\*|\+|-|%|,)\s*/';
// $cleaned_string = preg_replace($pattern, '$1', $input_string);
    // return $cleaned_string;
// }
// Example usage
// $input_string = "constraint=1 & facility_id= Primary Health Center || q1 = 5 && q3>5 && q4<6 && q7!= 8 && q9==80";
// $cleaned_string = remove_spaces_around_operators($input_string);
// echo $cleaned_string;
?>
<?php
	...
	$tmp_name = $_FILES['structure']['tmp_name'];
		...
		$project_id = $_POST['project_id'];
		$survey_Name_field = SpecialCharRemove(sanitizeInput($_POST['survey_name'],$conn));
		...
		if($surveyname==$survey_Name_field){
			...
			if($file_check=="1"){
				...
				if(!in_array($ext, $allowedExtensions)){
					$result = array("status"=>0,"msg"=>"Invalid Excel Type.");
					echo json_encode($result);
					exit();
				}
			...
			if($survey_id!=''){
				...
				$insertForm = mysqli_query($conn,"INSERT INTO survey SET survey_name='".$survey_name."',file_check='".$file_check."', project_id='".$project_id."',unique_id='".$unique_id."', user_id='".$_SESSION['user_id']."', client_id='".$_SESSION['client_id']."', category_id='".$category_ids."',other_specify='".$others."',questinnour_file='".$file_name."' ,created_at='".$createdAt."',form_version='1' ");
				$inserted_form_id = mysqli_insert_id($conn);
				
			}else{
				$insertForm = mysqli_query($conn,"INSERT INTO survey SET survey_name='".$survey_name."',file_check='".$file_check."',project_id='".$project_id."',unique_id='".$unique_id."', user_id='".$_SESSION['user_id']."', client_id='".$client_id."', category_id='".$category_ids."',other_specify='".$others."',questinnour_file='".$file_name."',created_at='".$createdAt."',form_version='V1' ");
				$inserted_form_id = mysqli_insert_id($conn);
			} 
			if($inserted_form_id=='' || $inserted_form_id==0){
				$result = array("status"=>3,"msg"=>"Something went wrong.");
				echo json_encode($result);
				exit();
			}
			...
			if ($file_check ==1){
					...
					if($sheetSno=='0'){
						...
						for ($row = 2; $row <= $highestRow; $row++) {
							...
							foreach ($rowData as $key => $rowvalue) {
								...
								$relevant = remove_spaces_around_operators($rowvalue['relevant']);
								$constraints = trim(safe_var($conn,$rowvalue['constraint']));
								//$constraints = $rowvalue['constraint'];
								...
								$calculation = $rowvalue['calculation'];
								...
								if ($type != ''){
									...
									if($language_id=="1"){
										...
										*/
										...
										//UNDER BEGIN GROUP NOTE QUESTION ADD
										if($type=="begin_group" && $question_name!=''){
											$note_begin_group ="insert into questions set question_name='".$question_name."',dictionary_label='".$question_name."',questions_type_id='16',field_name='note".$last_id."',category_name='note".$last_id."',question_input_type_id='".$question_input_type_id."',input_field_type='note',title='note".$last_id."',survey_id='".$inserted_form_id."',screen_no='".$screen_no."',sequence_no='".$sequence_no."',ref_table='".$groupid."',group_id='0'";
											$saveGroup=mysqli_query($conn,$note_begin_group);
											$last_idlg=mysqli_insert_id($conn);
										...
										//UNDER BEGIN GROUP NOTE QUESTION ADD
										if($type=="begin_group" && $question_name!=''){
											$note_begin_group ="insert into questions_language set language_id='1',question_id='".$last_idlg."', question_name='".$question_name."',dictionary_label='".$question_name."',questions_type_id='16',field_name='note".$last_id."',category_name='note".$last_id."',question_input_type_id='".$question_input_type_id."',input_field_type='note',title='note".$last_id."',survey_id='".$inserted_form_id."',screen_no='".$screen_no."',sequence_no='".$sequence_no."',ref_table='".$groupid."',group_id='0'";
											$saveGroup=mysqli_query($conn,$note_begin_group);
										...
										if($type=="begin_repeat"){ 
											...
											if($appearance=='onescreen'){
												 $appearanceone.=', appearance="onescreen"';
											} 
											...
											if(is_int($reptno) && $reptno>0){
												...
												$update = mysqli_query($conn,"UPDATE questions_language SET repeated='".$insert_group_id."',repeat_count='".$field_nameBgn."', parent_relevent='".$prelv."' $appearanceone $rosterRelevant WHERE question_id='".$qid_insert."' ");
											}
										...
										/////////////////////////////////
										if($type=="begin_group" && $question_name!=''){
											$last_idlg_note=$last_idlg;
											$qid_insertts=$last_idlg-1;
											$update = mysqli_query($conn,"UPDATE questions SET screened='".$qid_insertts."',screened_count='".$field_nameScreen."',group_id='".$insert_group_ids."' WHERE question_id='".$last_idlg_note."' ");

											$update = mysqli_query($conn,"UPDATE questions_language SET group_id='".$insert_group_ids."',screened='".$qid_insertts."' WHERE question_id='".$last_idlg_note."' ");
										...
										//UNDER BEGIN GROUP NOTE QUESTION other language ADD
										// if($type=="begin_group" && $question_name!=''){
										// 	$note_begin_group ="insert into questions_language set language_id='".$language_id."',question_id='".$question_id."', question_name='".$question_name."',dictionary_label='".$question_name."',questions_type_id='16',field_name='".$field_name."',category_name='".$field_name."',question_input_type_id='".$question_input_type_id."',input_field_type='note',title='".$field_name."',survey_id='".$inserted_form_id."',screen_no='".$screen_no."',sequence_no='".$sequence_no."',ref_table='".$groupid."',group_id='0'";
										// 	$saveGroup=mysqli_query($conn,$note_begin_group);
									...
									if(count($quest_labelarray)>0)
									{
										...
										// $field_name_lg=$questionId1->field_name;
										...
										if($type=="begin_group" && $question_name!=''){
											//$field_name_lg
											//$questionGroup_lg= getonefield($conn,'questions_group', 'id', 'group_name', $field_name);

											$questionGroup_lg=mysqli_query($conn,"select id from questions_group where group_name='".$field_name."' and survey_id='".$inserted_form_id."'");
											$question_group_idlg=mysqli_fetch_array($questionGroup_lg);
											$group_id_lg=$question_group_idlg['id'];

											$note_begin_grouplg ="insert into questions_language set language_id='".$language_ids."',question_id='".$question_id."', question_name='".$question_name."',dictionary_label='".$question_name."',questions_type_id='16',field_name='note".$question_id."',category_name='note".$question_id."',question_input_type_id='".$question_input_type_id."',input_field_type='note',title='note".$question_id."',survey_id='".$inserted_form_id."',screen_no='".$screen_no."',sequence_no='".$sequence_no."',ref_table='".$groupid."',group_id='".$group_id_lg."'";
											$saveGrouplg=mysqli_query($conn,$note_begin_grouplg);

											// $updatelgGroup = mysqli_query($conn,"UPDATE questions_language SET group_id='".$insert_group_ids."',screened='".$qid_insertts."' WHERE question_id='".$last_idlg_note."' ");
										...
										//UNDER BEGIN GROUP NOTE QUESTION ADD
											...
											//Start Other language repeated and group condition
											$group_id=0; 
											...
											//End Other language repeated and group condition
										}
									}
									$sequence_no=$sequence_no+1;

									//////////BEGIN GROUP QUESTION BLANK/////////////////////////
									if($type=="begin_group"){
										$update_ques_group="update questions set question_name='',dictionary_label='' where input_field_type='begin_group' and survey_id='".$inserted_form_id."'";
										mysqli_query($conn,$update_ques_group);

										$update_queslg_group="update questions_language set question_name='',dictionary_label='' where input_field_type='begin_group' and survey_id='".$inserted_form_id."'";
										mysqli_query($conn,$update_queslg_group);
			   ...
			   // $question_language_sql ="insert into questions_language set language_id='".$language_id."', question_id='".$question_id."' ,question_name='Your survey is complete. Do you want to submit?',question_description='Desc',questions_type_id='16',repeat_count='',question_input_type_id='',max_input='0',screen_no='1',input_field_type='note',read_only='',calculation='',ref_table='',group_id='0',sequence_no='".$sequence_no."',field_name='submit',validation_id='0',title='Submit',category_name='',parameters='',survey_id='".$inserted_form_id."', relevant='',constraints='',required='', constraint_msg='' , choice_filter='' , appearance='', choice_relation='',default_response='', paradata='', normal_group_id='', unique_id='', preserve='' ";
				$question_language_sql ="insert into questions_language set language_id='1', question_id='".$question_id."' ,question_name='You are at the end of record. Do you want to submit?',question_description='Desc',questions_type_id='16',repeat_count='',question_input_type_id='',max_input='0',screen_no='1',input_field_type='note',read_only='',calculation='',ref_table='',group_id='0',sequence_no='".$sequence_no."',field_name='submit',validation_id='0',title='Submit',category_name='',parameters='',survey_id='".$inserted_form_id."', relevant='',constraints='',required='', constraint_msg='' , choice_filter='' , appearance='', choice_relation='',default_response='', paradata='', normal_group_id='', unique_id='', preserve='' ";
				mysqli_query($conn,$question_language_sql);
			
				if(count($allLanguages)>0){
					$allLanguages = array_unique($allLanguages);
					foreach($allLanguages as $allLanguage){
						$question_language_sql ="insert into questions_language set language_id='".$allLanguage."', question_id='".$question_id."' ,question_name='You are at the end of record. Do you want to submit?',question_description='Desc',questions_type_id='16',repeat_count='',question_input_type_id='',max_input='0',screen_no='1',input_field_type='note',read_only='',calculation='',ref_table='',group_id='0',sequence_no='".$sequence_no."',field_name='submit',validation_id='0',title='Submit',category_name='',parameters='',survey_id='".$inserted_form_id."', relevant='',constraints='',required='', constraint_msg='' , choice_filter='' , appearance='', choice_relation='',default_response='', paradata='', normal_group_id='', unique_id='', preserve='' ";
						mysqli_query($conn,$question_language_sql);
					}
				}

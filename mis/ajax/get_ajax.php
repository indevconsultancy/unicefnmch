...
if (isset($_REQUEST['questionid']) && $_REQUEST['questionid'] != "" ) {
	$question_bank_id = mysqli_real_escape_string($conn, $_REQUEST['questionid']);
	$sqlquestionsd=mysqli_query($conn,"select * from question_bank where question_bank_id='" . $question_bank_id . "'");
	$dataquestiond=mysqli_fetch_object($sqlquestionsd);
	if($dataquestiond->user_id==$_SESSION['user_id'])
	{
	...
	if ($delQuestion) {
		...
		$result = array("status" => 0, "message" => "Not Authorized to Delete this Question");
	}

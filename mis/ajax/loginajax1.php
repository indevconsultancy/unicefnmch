...
function login($username,$password,$conn)
{
    ...
    $user_name=$username;
   // $storedpass=$password;
	//echo $password = hash('sha256',$storedpass);
    ...
    if(mysqli_num_rows($authentication_query)>0){
	  ...
	  if (password_verify($password,$hash_password)) {
		...
		$clientGet=mysqli_query($conn,"select logo_image from clients where id='".$login_data['client_id']."'");
		$client_data=mysqli_fetch_array($clientGet);
		//$logo_image=$client_data['logo_image'];
		  ...
		  $_SESSION['logo_image'] = $client_data['logo_image'];
		   session_regenerate_id(true);
...
if(isset($_REQUEST['username']) && isset($_REQUEST['password']))
	...
	if($_SESSION['login_attempts']<3)
	{
		...
		if($_SESSION['CODE']==$captcha){    
			...
			$result=login($_REQUEST['username'],$_REQUEST['password'],$conn);
				...
				{ 
			      if($result['roles']==4)
				  {
				   ...
				   $results = array("restatus" => 1, "roles" => 1);
                    ...
                    $error = "You have made ".$_SESSION['login_attempts']." unsuccessful attempt(s) out of 3 allowed attempts.";
			...
			{
				$error="You have completed ".$_SESSION['login_attempts']."  attempts. Your account has been locked, Kindly try again after 15 minutes";
				$results = array("status" => 0, "roles" => 0, "error"=> $error);
			}

...
$surveyid=$_REQUEST['ssidd'];
	...
	if ($getSurvey['status'] == 1) {
		...
		// $getEndGroupsQuestions = mysqli_query($conn,"SELECT question_id, dictionary_label, field_name, input_field_type, relevant,normal_group_id, repeat_count, repeated, screened_count , screened,parent_relevent,roster_relevent,appearance FROM `questions_language` WHERE survey_id='".$surveyid."' AND language_id='1' AND input_field_type in('end_group','end_repeat') AND (( ( screened!='' AND screened_count!='') || (input_field_type='end_group' AND repeated!='' AND repeat_count!='') ) || ( ( screened!='' AND screened_count!='') || (input_field_type='end_repeat' AND repeated!='' AND repeat_count!='') )) ");
		
	
		//$getbegainGroupsQuestions = mysqli_query($conn,"SELECT question_id, dictionary_label, field_name, input_field_type, relevant, normal_group_id, repeat_count, repeated, screened_count, screened, parent_relevent, roster_relevent, appearance FROM `questions_language` WHERE survey_id='".$surveyid."' AND language_id='1' AND input_field_type IN ('begin_group', 'begin_repeat') AND (((screened='' AND screened_count='') OR (input_field_type='begin_group' AND repeated='' AND repeat_count='')) OR ((screened='' AND screened_count='') OR (input_field_type='begin_repeat' AND repeated!='' AND repeat_count!='')));
	
	
	 // die();
		$getbegainGroupsQuestions = mysqli_query($conn,"SELECT question_id, dictionary_label, field_name, input_field_type, relevant, normal_group_id, repeat_count, repeated, screened_count, screened, parent_relevent, roster_relevent, appearance FROM `questions_language` WHERE survey_id='".$surveyid."' AND language_id='1' AND ( (input_field_type = 'begin_group' AND appearance = 'onescreen') OR input_field_type = 'begin_repeat')");
		
		
		while($endGroupQuestions = mysqli_fetch_array($getbegainGroupsQuestions, MYSQLI_ASSOC)){
			//echo "\n";
			 $endgqId = $endGroupQuestions['question_id'];
			  $endgfield_name = $endGroupQuestions['field_name'];
			 
			 
			 $screenGroup=mysqli_query($conn,"SELECT id,group_name,group_type FROM `questions_group` where survey_id='".$surveyid."' and group_name='".$endgfield_name."'");
			 $groupfield = mysqli_fetch_array($screenGroup, MYSQLI_ASSOC);
			 
			 if (empty($endGroupQuestions['screened']) && $endGroupQuestions['input_field_type'] == 'begin_group') {
				$endGroupQuestions['screened'] = $groupfield['id'];
			}
			if (empty($endGroupQuestions['screened_count']) && $endGroupQuestions['input_field_type'] == 'begin_group') {
				$endGroupQuestions['screened_count'] = $groupfield['group_type'];
			}
			if (!empty($endGroupQuestions['repeated']) && $endGroupQuestions['input_field_type'] == 'begin_repeat') {
				$endGroupQuestions['repeated'] = $groupfield['id'];
			}
			// if (empty($endGroupQuestions['repeat_count']) && $endGroupQuestions['input_field_type'] == 'begin_repeat') {
				// $endGroupQuestions['repeat_count'] = $groupfield['group_type'];
			// }
			...
			//$getQuess = mysqli_query($conn,"SELECT question_id FROM questions where survey_id='".$surveyid."' AND input_field_type not in('end_group','end_repeat') AND dictionary_label!='' AND ((screened='start') || (screened_count='' || screened_count IS null)  )  AND (repeated='0' || repeated='' || repeated='start' || repeated IS null) AND question_id<'".$endgqId."' order by question_id desc limit 1  ");
			
			//echo "SELECT question_id FROM questions where survey_id='".$surveyid."' AND input_field_type not in('end_group','end_repeat') AND dictionary_label!='' AND ((screened='start') || (screened_count='' || screened_count IS null)  )  AND (repeated='0' || repeated='' || repeated='start' || repeated IS null) AND question_id<'".$endgqId."' order by question_id desc limit 1  ";
			///09.09.2024
			//$getQuess = mysqli_query($conn,"SELECT question_id FROM questions where survey_id='".$surveyid."' AND input_field_type not in('end_group','end_repeat') AND dictionary_label!='' AND ((screened='start') || (screened_count='' || screened_count IS null)  )  AND (repeated='0' || repeated='' || repeated='start' || repeated IS null) AND question_id<'".$endgqId."' order by question_id desc limit 1  ");
			////////////////
			$getQuess = mysqli_query($conn,"SELECT question_id FROM questions where survey_id='".$surveyid."' AND input_field_type not in('end_group','end_repeat') AND dictionary_label!='' AND ((screened='start') || (screened_count='' || screened_count IS null)  )  AND (repeated='0' || repeated='' || repeated='start' || repeated IS null) AND question_id<='".$endgqId."' order by question_id desc limit 1  ");
		...
		// echo "<pre>";
		// print_r(json_encode($hasScreens));
		// die();
		...
		while($language_data = mysqli_fetch_object($languageQuery)){
			...
			while($groups = mysqli_fetch_object($groupQuery)){
				...
				while($screen = mysqli_fetch_object($screenQuery)){
					...
					//$batch['question_position_id'] = $question_index;
				 ...
				 //	print_r($batches);
					...
					//echo $batches->parent_relevent;
					 if($batches->screened!="" && $batches->relevant!=""){  //$batches->appearance!="" condition add on date 22-july-2024
						...
						$anonymousq['question_position_id'] = $question_index+1;
					...
					if(array_key_exists($batches->question_id, $hasScreens)){
						...
						foreach($allqs as $q){
							...
							// if($checkquinque==$batches->screened){
								// $checkquinque=$batches->screened+1;
							// }else{
								// $checkquinque=$batches->screened;
							// }
							//echo "sonu".$sfdfdf;
							...
							$anonymousq['question_input_type'] = "anonymous";
							//$anonymousq['question_position_id'] = $question_index+1;
							$anonymousq['screened'] = $q['screened'];	 // $batches->screened;					
							$anonymousq['screened_count'] = $q['screened_count'];  //$batches->screened_count;
							$anonymousq['repeated'] = $q['repeated']; //$batches->repeated; 
							$anonymousq['repeat_count'] =$q['repeat_count']; // $batches->repeat_count;    
							$anonymousq['relevant'] = $anonymsconditions?:"";
							$anonymousq['parent_relevent'] = $batches->parent_relevent; //$q['parent_relevent'];
							$anonymousq['roster_relevent'] = $batches->roster_relevent; //$q['roster_relevent'];

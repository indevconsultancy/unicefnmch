<?php 
session_start();
header("Content-Security-Policy: default-src 'self' 'unsafe-inline'; img-src 'self' data:; connect-src 'self' 'unsafe-inline'; font-src 'self' data: https://fonts.gstatic.com https://cdn.jsdelivr.net; script-src 'self' 'unsafe-inline' https://mquad.org/mis/js/ https://cdn.jsdelivr.net; object-src 'none'; style-src 'self' 'unsafe-inline'; style-src-elem 'self' 'unsafe-inline' https://fonts.googleapis.com/ https://cdn.jsdelivr.net;  frame-ancestors 'none';");
...
function checkUservailid($qry1,$value1,$qry2,$value2,$conn)
{
	$qrys='';
	 if($qry2!='')
	 {
		 $qrys=" and $qry2='".$value2."'";
	 }
	
	$sn=mysqli_query($conn,"select user_id as id from users where $qry1='".$value1."' $qrys")or die(mysqli_error($conn));
	$dn=mysqli_fetch_object($sn);
    $result=false;
	if($dn->id>0)
	{

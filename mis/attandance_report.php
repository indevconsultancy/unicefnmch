...
if (isset($_REQUEST['search'])) {
    ...
    if (isset($_REQUEST['fromDate']) && isset($_REQUEST['toDate'])) {
		...
		$ssdate=$startDate->format('Y-m-d');
        $endDate = new DateTime($_REQUEST['toDate']);
		$eedate=$endDate->format('Y-m-d');
    ...
    $qry.= " AND user_types LIKE '%unicef%'";
	}
}
else {
    // Default query if no search is performed
        ...
        <div class="col-lg-2 col-md-3 col-sm-12">
            <div class="form-group"> 
                ...
                while($dataUserType = mysqli_fetch_object($qryUserType)) {
					...
					else if($dataUserType->user_types=='unicef')
					{
						$selected='selected';
					}
					else {
				...
				<div class="col-sm-12">
					...
					<section class="panel p-1">
						...
						<?php
							...
							$usersStmt = mysqli_query($conn,"SELECT DISTINCT monitor_name FROM survey_data_monitoring where 1=1 and monitor_name not in('Dr.Mohit Bhardwaj','Dr Deepika Agrawal','GAGAN GAUTAM','Dr. Shrankhala Mishra') $qry $qry1 ORDER BY monitor_name ASC");

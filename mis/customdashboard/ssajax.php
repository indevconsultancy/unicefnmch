...
<?php 
	if(isset($_POST['chartType'])){
		...
		$response = dynamic_graph($conn, $survey_id, $chartType, $cType, $legendColors, $field_name, $gquestionFName,"",$others);	
...
if(isset($_POST['process']) && $_POST['process']=="UPDATE-GRAPH"){
	...
	$sqlUpdate = "UPDATE surveydashboard SET $graphDescriptionsql title='".$graphTitle."'  where id='".$graphId."' ";
	$resData = mysqli_query($conn,$sqlUpdate);
...
if(isset($_POST['process']) && $_POST['process']=="EDIT-GRAPH-DATA"){
	 $graphIds = mysqli_real_escape_string($conn, $_POST['graphIds']);
	 $survey_id = mysqli_real_escape_string($conn, $_POST['survey_id']);
	 $sqlGraph=mysqli_query($conn,"SELECT id,title,description  FROM `surveydashboard` WHERE `survey_id` = '".$survey_id."' and id='".$graphIds."' and status='0'");
	 $graphData=mysqli_fetch_array($sqlGraph);
	 $title = htmlspecialchars($graphData['title'], ENT_QUOTES, 'UTF-8');
    $description = htmlspecialchars($graphData['description'], ENT_QUOTES, 'UTF-8');
	?>
	<div class="mb-3">
		<label for="graphTitle" class="form-label">Title:</label>
		<input type="hidden" class="form-control" id="graphId" name="graphId" value="<?=$graphIds;?>"/>
		<input type="text" class="form-control" id="graphTitle" name="graphTitle" value="<?=$title;?>"/>
	</div>
	<div class="mb-3">
		<label for="graphDescription" class="form-label">Description:</label>
		<textarea class="form-control" id="graphDescription" name="graphDescription" rows="5"><?=$description;?></textarea>
	</div>

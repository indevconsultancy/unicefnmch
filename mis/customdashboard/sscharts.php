...
function stackColumnCharts($chartId,$title, $categories, $seriesData, $graphPlotOptions, $chartType, $colors,$others=[]){
	...
	return $options =" <script> var customChartOptions = {
			...
			exporting:{ 
				enabled:true, 
				menuItemDefinitions: {
				downloadXLS: null // Remove the XLS download option
			},
			},
			plotOptions: {
				column: {
...
function decrypt_field($survey_id,$conn) {
	...
	while($surData = mysqli_fetch_object($getSurveyData)){
		...
		$full_json = $surData->survey_data_json;
...
function dynamic_graph($conn, $survey_id, $chart_name, $chart_type, $colors, $field_name, $groupby="", $chartId="",$others=[]){
	...
	if($chartType=="pie"){ 
		...
		$totalCount="(N=".array_sum($totalRecords).")";
		$clrs = implode("','",$colors);
		if(count($legendColors)>0){ 
			$clrs = implode("','",$legendColors);
			$colors = $legendColors; 
		}
		$cOptions = basicPieChart($chartId,$totalCount,$series_final_pie_charts,$clrs,$others);	
		return $resArr = array(
			'categories' => '',
	...
	if($chartType=="basicClumnAdvance" || $chartType=="stackClumnAdvance" || $chartType=="stackClumnAdvance100"){ 
		...
		$totalCount="(N=".array_sum($advtotRecords).")";
		...
		$cOptions = stackColumnCharts($chartId,$totalCount, $categories, $seriesdata, $graphPlotOptions, $cType, $clrs, $others);
		return $resArr = array(
			'totalCount' =>$totalCount,
			'categories' => $categories,
	...
	if($chartType=="splitBubbleAdvance"){
		...
		$cOptions = bubbleChart($chartId,$totalCount,$series_serdd,$clrs,$others);
		return $resArr = array(
			'totalCount' => $totalCount,
			'categories' => $categories,
	...
	if($chartType=="RadialAdvance"){
		...
		*/
		...
		$cOptions = radialChart($chartId,$totalCount,$cType,$categories,$series_serdd,$clrs, $others);
		return $resArr = array(
			'categories' =>$categories,
			'totalCount' =>$totalCount,
			'dataseries' =>$series_serdd,
	...
	if($chartType=="spiderAdvance"){
		...
		*/
		...
		$cOptions = spiderChart($chartId, $totalCount, $cType, $categories, $seriesdata_serdd, $clrs, $others);
		return $resArr = array(
			'categories' =>$categories,
			'totalCount' =>$totalCount,
	...
	if($chartType=="basicbar" || $chartType=="basicline" || $chartType=="basiccolumn" || $chartType=="basicarea"){ 
		...
		$cOptions = basicChart($chartId, $totalCount, $ctype, $final_bar_charts_series, $clrs, $others);
		return $resArr = array(
			'categories' => '',
			'totalCount' => $totalCount,

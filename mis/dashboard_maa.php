...
$qryreportingp = mysqli_query($conn, "select reporting_period from maa_monthly_report order by reporting_period desc limit 0,1");
$datareportingp = mysqli_fetch_object($qryreportingp);
$month_year = $datareportingp->reporting_period;

$date = DateTime::createFromFormat('Y-m-d', $month_year); // Format it as "F-Y" where "F" gives the full month name and "Y" gives the year
...
if (isset($_REQUEST['Submit']) && $_REQUEST['Submit'] == 'Submit') {
	if (isset($_REQUEST['reporting_period']) && $_REQUEST['reporting_period'] != '') {
		...
		$date2 = DateTime::createFromFormat('Y-m-d', $qryperiod); // Format it as "F-Y" where "F" gives the full month name and "Y" gives the year
...
<section id="main-content">
	<section class="wrapper">
			...
			<div class="row breadcrumb">
				...
				<div class="col-lg-2">
					<select class="form-control" name="reporting_period" style="margin-top:10px;">
						...
						for ($month = 4, $year = $financial_year_start; $month <= 24; $month++) {
							...
							$reporting_periodss = $current_year . "-" . $current_month . "-01";
		...
		$qrycounts = mysqli_query($conn, "select sum(total_nos_asha) as total_nos_asha, sum(ashas_oriented_iycf) as tot_nos_asha_orented_iycf_meeting , sum(mothers_meetings_held) as tot_nos_asha_conducted_mothermeeting, sum(facilities_given_maa_award) as tot_nos_facility_given_maa_award from maa_monthly_report where reporting_period='" . $qryperiod . "' $qrydistrict");
						...
						<div class="card">
							...
							<div class="card-body">
								<table id="example" class="display nowrap table-bordered" style="width:100%">
									...
									<tbody>
										<?php
										$sql_table = "Select id, district, block, district_name, block_name, reporting_period, nodal_officer_name, nodal_officer_contact, total_nos_asha, ashas_oriented_iycf, ashas_with_infokit, ashas_conducted_meetings, ashas_received_incentive, total_mothers, pregnant_mothers, lactating_mothers, mothers_participated_meeting, iycf_master_trainers, mothers_meetings_held, staff_sensitized, delivery_points_sensitized, facilities_given_maa_award FROM maa_monthly_report WHERE reporting_period='" . $qryperiod . "'";
										$table_qry = mysqli_query($conn, $sql_table);
										while ($table_data = mysqli_fetch_object($table_qry)) {
											$reporting_period = DateTime::createFromFormat('Y-m-d', $table_data->reporting_period); // Format it as "F-Y" where "F" gives the full month name and "Y" gives the year
										...
										?>
											<tr>
												...
												<td><?= $table_data->ashas_oriented_iycf ?></td>
												<td><?= $table_data->ashas_with_infokit ?></td>
												<td><?= $table_data->ashas_conducted_meetings ?></td>
												<td><?= $table_data->ashas_received_incentive ?></td>
												<td><?= $table_data->total_mothers ?></td>
												<td><?= $table_data->pregnant_mothers ?></td>
												<td><?= $table_data->lactating_mothers ?></td>
												<td><?= $table_data->mothers_participated_meeting ?></td>
												<td><?= $table_data->iycf_master_trainers ?></td>
												<td><?= $table_data->mothers_meetings_held ?></td>
												<td><?= $table_data->staff_sensitized ?></td>
												<td><?= $table_data->delivery_points_sensitized ?></td>
												<td><?= $table_data->facilities_given_maa_award ?></td>
												<td><?= date("d-M-Y", strtotime($table_data->reporting_period)) ?></td>
											</tr>
										<?php } ?>
									</tbody>
									<tfoot>
...
<script>
	...
	$district_map = mysqli_query($conn, "SELECT count(id) as survey_count, district_name,district_id,map_code FROM maa_monthly_report where reporting_period='" . $qryperiod . "' GROUP BY map_code");
	$distictMap = '';
...
$district_reported_qry = mysqli_query($conn, "SELECT count(distinct(district)) as survey_count FROM maa_monthly_report where reporting_period='" . $qryperiod . "'");
...
<script>
	...
	$form_wise_qry = mysqli_query($conn, "SELECT count(id) as survey_count, district_name,district as map_code FROM maa_monthly_report where reporting_period='" . $qryperiod . "' GROUP BY map_code");
	$formData = '';
...
$m = 0;
$datasss = '';
$sqlmomreporting = mysqli_query($conn, "select reporting_period,sum(total_nos_asha) as asha ,sum(pregnant_mothers) as pw,sum(lactating_mothers) as lw,sum(facilities_given_maa_award) as maaAward from maa_monthly_report group by reporting_period");
while ($datamon_reporting = mysqli_fetch_object($sqlmomreporting)) {
	if ($m > 0) {
		$datasss .= ',';
	}
	$date1 = DateTime::createFromFormat('Y-m-d', $datamon_reporting->reporting_period); // Format it as "F-Y" where "F" gives the full month name and "Y" gives the year

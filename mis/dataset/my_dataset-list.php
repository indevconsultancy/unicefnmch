			...
			<div class="col-lg-12">
				<nav aria-label="breadcrumb">
					<ol class="breadcrumb">
						...
						<li class="breadcrumb-item" aria-current="page"><i class="fa fa-bar-chart"></i></i>My Dataset(s)</li>
					</ol>
				</nav>
				<!-- Filter End-->
			</div>
										...
										<div class="side-data">
											<ul>
												...
												while ($datacategory = mysqli_fetch_array($datassqry)) {
													...
													$sqlcatdata = "SELECT COUNT(dataset_id) as total FROM project_datasets where find_in_set('" . $datacategory['category_id'] . "',thematic_area_id) and project_datasets.status='1'  $qry";
													$sqlcategory = mysqli_query($conn, $sqlcatdata);
													while ($row = mysqli_fetch_array($sqlcategory)) {
												?>

														<li>
															<a href="my_dataset-list.php?category_id=<?= $datacategory['category_id'] ?>&published_date=&client_name=&searchdata="><span><?php echo $datacategory['category_name']; ?></span> <span class="badg"><?php echo $row['total']; ?></span></a>
														</li>
												<?php }
												} ?>
											</ul>

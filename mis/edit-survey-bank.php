...
if (isset($_REQUEST['upload'])) {
	...
	$survey_title = sanitizeInput($_POST['survey_title'],$conn);
	$survey_bank_id = $_POST['survey_bank_id'];
	...
	if ($questionnaire_type != "Link") {
		...
		// Sanitize input to remove any unwanted HTML tags
		$questionnaire_link = filter_input(INPUT_POST, 'questionnaire_link', FILTER_SANITIZE_URL);

		if (!filter_var($questionnaire_link, FILTER_VALIDATE_URL)) {
			$_SESSION['status_error'] = "Invalid questionnaire link";
			$_SESSION['status_error_code'] = "warning";
			exit();
		} else {
			$questionnaire_file = htmlspecialchars($questionnaire_link, ENT_QUOTES, 'UTF-8');
		}
	}
	...
	if ($insertcontribute) {
		$_SESSION['status'] = "Tool has been successfully updated";
			...
			<div class="col-lg-12">
				...
				<nav aria-label="breadcrumb">
					<ol class="breadcrumb">
						...
						<li class="breadcrumb-item" aria-current="page"><i class="fa fa-plus"></i>Update Tool Archives</li>
					</ol>
				</nav>
			</div>
		</div>
					...
					<div class="panel-body">
						<div class="form">
							<form class="form-horizontal" id="myForm" method="post" enctype="multipart/form-data">
								...
								<div class="col-lg-offset-2 col-lg-12 mt-xl  text-end">

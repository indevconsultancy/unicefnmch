...
<?php 
if($_REQUEST['formType'] == 'webForm'){
	$formType = "webForm";
}
if($_REQUEST['formType'] == 'replaceForm'){
	$formType = "replaceForm";
}
	...
	<script>
		swal.fire({
			...
			<?php if($formType == 'webForm') { ?>
			window.location.href="<?=base_url();?>add-question-web-v1.php?survey_id=<?=@$_GET['survey_id'];?>&page=<?=$pages?>"
			<?php } 
			if($formType == 'replaceForm') { ?>
			window.location.href="<?=base_url();?>replace_webform.php?survey_id=<?=@$_GET['survey_id'];?>&page=<?=$pages?>"
			<?php } ?>
			//location.reload();
		});
	</script>
...
<script>
	swal.fire({
		...
		window.location.href="<?=base_url();?>survey-list.php?survey_id=<?=@$_GET['survey_id'];?>&page=<?=$pages?>"
		//location.reload();
	});
</script>
<?php  }  ?>
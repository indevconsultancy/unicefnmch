...
$get_sdmon = mysqli_query($conn,"SELECT survey_data_monitoring_id,survey_id, survey_data_json_export, full_json,survey_name  FROM survey_data_monitoring WHERE survey_name_id = '".$survey_id_exported."' and language_id='".$language_id."' order by survey_data_monitoring_id asc");
while($sdmon = mysqli_fetch_object($get_sdmon)){
         $survey_data_json_export=$sdmon->survey_data_json_export;
        $surveyid = $sdmon->survey_id;
		$survey_name = $sdmon->survey_name;
         $jsondata = json_decode($survey_data_json_export, true);
...
// echo "<h2>Value</h2>";
$get_sdmon = mysqli_query($conn,"SELECT survey_data_monitoring_id,survey_id, survey_data_json_export, full_json  FROM survey_data_monitoring WHERE survey_name_id = '".$survey_id_exported."' and language_id='".$language_id."' order by survey_data_monitoring_id asc");
$counter=2;
while($sdmon = mysqli_fetch_object($get_sdmon)){
        $survey_data_json_export=$sdmon->survey_data_json_export;
...
$sheet_no=0;
$counter_r=2;
$get_repeated_sdmon = mysqli_query($conn,"SELECT survey_data_monitoring_id,survey_id, survey_data_json_export, full_json  FROM survey_data_monitoring WHERE survey_name_id = '".$survey_id_exported."' order by survey_data_monitoring_id asc");
            ...
            if($fulljsonKey!="survey_data" && $fulljsonKey!="hint_record" && $fulljsonKey!="error_record"){
                ...
                foreach($repeated as $rptkey=>$repeatedq){
                    ...
                    foreach($repeatedq as $repeatedqs){
						...
						if($option_id!=""){
							$getoption = mysqli_query($conn,"SELECT option_name FROM options_language WHERE question_id='".$question_id."' AND language_id='".$language_id."' AND option_sequence='".$option_id."' ");

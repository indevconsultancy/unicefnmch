...
// Fetch survey data
$getMondatas = mysqli_query($conn, "SELECT survey_data_monitoring_id, survey_id, survey_data_json, full_json, survey_status, users.name as full_name, termination_reason,survey_name  FROM survey_data_monitoring LEFT JOIN users ON users.user_id = survey_data_monitoring.user_id WHERE survey_name_id='$surveyid' AND survey_status != '7' limit 0,3");
$counter = 2;
...
while ($mdata = mysqli_fetch_object($getMondatas)) {
    ...
    foreach ($survey_data as $surveydata1) {
		...
		if($input_field_types[$surveydata1->field_name]=='select_multiple'){
				...
				{
					$field_names[]=$datamulitiques->option_name;
					if(in_array($datamulitiques->option_name,$optionData))
					{
					$opt_value12='1';	
					}
					else{
					...
					$groupoptName1=$questionNames.'/'.$datamulitiques->option_name;
					$jsn_data[$groupoptName1] = $opt_value12;
	...
	foreach($scrn as $srcn1){
			...
			foreach ($group_data[0] as $groupdata) {
				$grpqnName=getonefield($conn, 'questions', 'input_field_type', 'question_id', $groupdata->question_id);
				...
				if($grpinptutt=='select_multiple'){
							...
							while($datamulitiques1=mysqli_fetch_object($sqlmultiques1))
							{
								...
								if(in_array($datamulitiques1->option_name,$optionData))
								{
								$opt_value23='1';	
								}
								else{
								...
								$groupoptName=$grpqnName.'/'.$datamulitiques1->option_name;
...
$qryQuestion_level = mysqli_query($conn, "SELECT question_name,question_id,input_field_type FROM questions_language WHERE field_name IN(".$questionfields.") AND survey_id='" .$surveyid. "' and language_id='" .$language_id. "'");

while ($dataQuestion = mysqli_fetch_object($qryQuestion_level)) 
		{
			$dataQuestions[]= $dataQuestion->question_name;
			if($dataQuestion->input_field_type=='select_multiple')
			{
				$sqlmultiques2=mysqli_query($conn, "select option_name,option_sequence,language_id from options_language where question_id='".$dataQuestion->question_id."' and language_id='".$language_id."'");
							while($datamulitiques2=mysqli_fetch_object($sqlmultiques2))
							{
								$dataQuestions[]= $datamulitiques2->option_name;
							}
			}
		} */

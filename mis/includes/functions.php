...
function sanitizeInput($data, $conn) {
	if (is_array($data)) {
        // If the data is an array, loop through each element
...
function send_mail_function($mailto,$message,$subject) {
		...
		$message = base64_decode($message) . '<br><br>Warm Regards,<br> Unicef Team <br><br>'; 
		// $subject = base64_decode($subject);
...
function uniqueId($name,$id){
 ...
 function paginate($item_per_page, $current_page, $total_records, $total_pages, $page_url)  {
	...
	function sendNotification($firebase_token,$activitis,$message,$json)
    {
       ...
       $path="https://unicef.indevconsultancy.in/firebase/?regId=$firebase_token&title=$activitis&message=$message&json=$json&push_type=individual";
...
function getDistrictsbyName($conn, $value)
{
    $sn = mysqli_query($conn, "select  district_code  as field from districts where local_name like'%" . $value . "%' ") or die(mysqli_error());
    $dn = mysqli_fetch_object($sn);
    return ($dn->field);
}
function getBloksbyName($conn, $value)
{
   $sn = mysqli_query($conn, "select  block_code  as field from blocks where local_name like'%" . $value . "%' ") or die(mysqli_error());
    $dn = mysqli_fetch_object($sn);
    return ($dn->field);
}

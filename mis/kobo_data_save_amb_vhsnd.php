            ...
            if (!in_array($key, $excludeKeys)) {
                if (strpos($key, '/') !== false) {
                    ...
                    if (array_key_exists($group, $groupMap)) {
						...
						if (strpos($key, 'namemm') !== false) {
							$userOutput['monitor_name'] = $resultsArray[$key] ?? null;
						}
						if (strpos($key, 'District') !== false) {
							if($resultsArray[$key]=='Purnia')
							{
								$district='Purnea';
							}
							else {
								$district=$resultsArray[$key];
							}
							$userOutput['m_name']=$district;
							$userOutput['district_id'] = getDistrictsbyName($conn,$district);
						}
						if (strpos($key, 'Block') !== false) {
							$userOutput['org']=$resultsArray[$key];
						...
						if (strpos($key, 'type_mon') !== false) {
							if($resultsArray[$key]=='coe')
							{
								$userOutput['user_types'] =  'aiims';
							}
							else {
							$userOutput['user_types'] =  $resultsArray[$key];
							}
						}
						/*

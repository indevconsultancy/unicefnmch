...
if (isset($_REQUEST['search'])) {
	...
	if (isset($_REQUEST['survey']) && $_REQUEST['survey'] != '') {
		$qry .= " AND ((survey_data_monitoring.survey_id='" . $_REQUEST['survey'] . "') or  (survey_data_monitoring.survey_data_id='" . $_REQUEST['survey'] . "'))";
	}
			...
			<div class="col-lg-12">
				<nav aria-label="breadcrumb">
					<ol class="breadcrumb">
						...
						<li class="breadcrumb-item active" aria-current="page"><i class='fa fa-eye'></i>Review Data List</li>
					</ol>
				</nav>
			</div>
		</div>
				...
				<div class="row filter_css res-filter clearfix g-1">
					...
					<div class="col-lg-2 col-md-2 col-sm-12">
						<div class="form-group">
							<input type="text" class="form-control" id="survey" name="survey" value="<?= @$_REQUEST['survey'] ?>" placeholder="Record ID">
						</div>
					</div>
					<div class="col-lg-2 col-md-2 col-sm-12">
						<div class="form-group">
			...
			<div class="row">
				<div class="col-sm-12">
						...
						<div class="col-lg-4 col-md-4 col-sm-12 col-xs-12">
							<div class="info-box green-bg">
								...
								<div class="count">
									...
									// echo '<span class="text-left tooltips" data-bs-placement="right" data-bs-toggle="tooltip" data-bs-html="true" title="<span style=\'color:#449a97;font-weight: normal;\'>Terminated: ' . $datater['tot_terminated'] . '</span><br><span style=\'color:#449a97;font-weight: normal;\'>Sent for review: ' . $datasend['send_for_review'] . '</span>">' .
										// $data['acc_total'] .
										// '</span>';
					...
					<section class="panel p-1">
						<div class="">
							<header class="table-title d-flex justify-content-between align-items-center">Form Name: <?php echo $row1['survey_name']; ?> <?php if ($_SESSION['role_id'] != '3') { ?>|| Client Name: <?php echo $row1['client_name']; ?> <?php } ?> || Total Records: <?= $total_record ?>
								...
								<tbody>
									...
									if (mysqli_num_rows($getSurvey) > 0) {
										while ($survey = mysqli_fetch_array($getSurvey)) {
											...
											$DecryptedJson =  $survey['full_json'];
									...
									?>
											<tr>
												...
												<td><?php if ($sequence_unique_id != '') {
													...
													?></td>
												<td>
													<span class="label label-success" data-bs-toggle="tooltip" data-bs-html="true" data-bs-placement="top" title="App Version </br> Device: <?= $device_name ?> </br> OS Version: <?= $os_version ?> "> <?php if($app_version=='web') { echo "web"; } else {?> v<?= $app_version; ?> <?php } ?></span> 
													&nbsp;<?= $survey['name'] ?> (<?= $survey['username'] ?>)

												</td>
												<td><?= date('d-M-Y,H:i:s', strtotime($survey['created_on'])); ?></td>
												<td>
													<?php
													...
													?>
														<span class="label label-warning">Resubmitted</span>
													<?php
												...
												<td>
													<a href="exportparadata.php?id=<?= $survey['survey_data_monitoring_id']; ?>" data-bs-placement="top" data-bs-toggle="tooltip" class="btn-sm btn-success tooltips" title="Download Paradata"><i class="fa fa-download" aria-hidden="true"></i></a>
													<a href="view_survey.php?id=<?= $survey['survey_data_monitoring_id']; ?>" class="btn-primary btn-sm" data-bs-toggle="tooltip" data-bs-placement="top" title="View Record"><i class="fa fa-eye"></i></a>
...
<?php
$result = mysqli_query($conn, "SELECT created_on,survey_id,survey_name,latitude,longitude,clients.name as client_name,users.username,users.name,monitor_name,start_date_time FROM survey_data_monitoring left JOIN users ON survey_data_monitoring.user_id=users.user_id left join clients on survey_data_monitoring.client_id=clients.id left JOIN survey_status_master ON survey_data_monitoring.survey_status=survey_status_master.id where survey_name_id='" . $surveyid . "' $qry $client_qry");
	...
	function initMap() {
		...
		var infoWindowContent = [
			...
			?>['<div class="info_content">' +
							...
							<?php if ($row['monitor_name']!= '') { ?> '<p style="color:#394A59;">Collected By: <?php echo $row['monitor_name']; ?></p>' + <?php } else { ?> '<p style="color:#394A59;">User Name: <?= $row['name']; ?> (<?php echo $row['username']; ?>)</p>' + <?php } ?> 
							'<p style="color:#394A59;">Form Id: <?php echo $row['survey_id']; ?></p>' +
							'<p style="color:#394A59;"><?php echo date('d-M-Y,H:i:s', strtotime($row['start_date_time'])); ?></p>' + '</div>'],

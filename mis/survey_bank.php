...
if (isset($_REQUEST['searchdata'])) {
	...
	if (isset($_REQUEST['from_date']) &&  trim($_REQUEST['tool_study_year'] != '')) {
		$title = mysqli_real_escape_string($conn, trim($_REQUEST['from_date']));

		$userqry .= " AND tool_study_year_from like'%" . $title . "%'";
	}
	...
	if ($from_date != '' && $to_date != '') {
		...
		$userqry .= " AND ((tool_study_year_from <= '$to_date' AND tool_study_year_to >= '$from_date') 
                    OR (tool_study_year_from >= '$from_date' AND tool_study_year_from <= '$to_date')
                    OR (tool_study_year_to >= '$from_date' AND tool_study_year_to <= '$to_date'))";
	} elseif ($from_date != '') {
		$from_date = mysqli_real_escape_string($conn, $from_date);
		$userqry .= " AND (tool_study_year_from >= '$from_date' OR tool_study_year_to >= '$from_date')";
	} elseif ($to_date != '') {
		$to_date = mysqli_real_escape_string($conn, $to_date);
		$userqry .= " AND (tool_study_year_from <= '$to_date' OR tool_study_year_to <= '$to_date')";
	}
}
?>
<?php
			...
			<div class="col-lg-12">
				...
				<nav aria-label="breadcrumb">
					<ol class="breadcrumb">
						...
						<li class="breadcrumb-item" aria-current="page"><i class="fa fa-list"></i></i>Show Tools</li>
						<li class="breadcrumb-item" aria-current="page"> </i></i>Total Tool(s): <?= $total_record  ?></li>
						...
						<div class="row g-2">
							<div class="col-sm-4">
								<div class="search-widget">
									<input id="survey_title" class="form-control" type="text" name="survey_title" placeholder="Name	" title="Name" data-bs-toggle="tooltip" data-bs-placement="top" aria-label="Search all data" value="<?= $_REQUEST['survey_title'] ?>">
								</div>
							</div>
							<div class="col-sm-2">
								<input type="text" name="from_date" id="from_datepicker" value="<?= @$_REQUEST['from_date'] ?>" placeholder="From Year" class="form-control">
							</div>
							<div class="col-sm-2">
								...
								<input type="text" name="to_date" id="to_datepicker" placeholder="To Year" value="<?= @$_REQUEST['to_date'] ?>" class="form-control">
								<!-- </div> -->
							</div>
							<div class="col-sm-2">
						...
						?>
								...
								<div class="survey-box" id="tools_id-<?= $contribute_row['id']; ?>">
									<div class="survey-container">
										<div class="survey-heading titled row">
											<div class="col-md-8 col-sm-12">
												<h4 class="mb-0"><?php echo $contribute_row['survey_title']; ?>
											</div>
											<div class="col-md-4 col-sm-12">
												<span style="float: right;color:white;" class="access-type"><?= $tool_access; ?></span>
												<!-- <?php //if ($contribute_row['user_id'] == $user_id) { 
														?>
																<a type="submit" href="edit-survey-bank.php?eid=<?php //$contribute_row['id'] 
																												?>" class="btn btn-success">Edit</a>
																<a href="javascript();" data-id="<?php //$contribute_row['id']; 
																									?>" class="btn btn-danger deltools">Delete</a>
															<?php //} 
															?> -->
											</div>
											</h4>
									...
									<div class="survey-container">
										...
										<div class="survey-data">
											<?php
											$sqlcdata = "SELECT GROUP_CONCAT(category_name SEPARATOR ', ') as category_name FROM `categories` WHERE `category_id` IN (" . $category_id . ") ";
									...
									<div class="survey-container">
										...
										<div class="survey-data">
											<p class="py-2 mb-0">
												...
												// echo
												// $tool_archive_status;
												if ($tool_archive_status == '1') { ?>
													<span class="label label-success">Approved</span>
												<?php } else if ($tool_archive_status == '2') { ?>
									...
									<div class="survey-container">
										<div class="survey-heading">
											<p><i class="fa fa-download"></i> Tool: </p>
										</div>
										<div class="survey-data">
											<?php if ($contribute_row['questionnaire_type'] == "File") { ?>
												<p><a href="upload_data_file/tools_archive_datafile/<?= $cid; ?>/<?php echo strip_tags($contribute_row['uploaded_questionnaire']); ?>" target="_blank"><?php echo strip_tags($contribute_row['uploaded_questionnaire']); ?></a></p>
											<?php } else { ?>
												<p><a href="<?php echo strip_tags($contribute_row['uploaded_questionnaire']); ?>" target="_blank"><?php echo strip_tags($contribute_row['uploaded_questionnaire']); ?></a></p>
											<?php }  ?>
										</div>
									</div>
									<div class="row">
										<div class="col-md-12" style="margin-top: 15px;">
											<div class="pull-right">
												<a href="view_tools.php?id=<?= $contribute_row['id']; ?>" class="label label-primary btn-primary"><i>View More</i></a>
											</div>
										</div>
									</div>
									...
									<div class="side-data">
										<ul>
											...
											if ($_SESSION['role_id'] == 1) {
												...
												$userwise = "and (survey_bank.user_id='" . $user_id . "' or tool_archive_status='1')";
											}
											...
											while ($datacategory = mysqli_fetch_array($datassqry)) {
												...
												$sqlcatdata = "SELECT COUNT(id) as total FROM survey_bank where find_in_set('" . $datacategory['category_id'] . "',category_id) $userwise and  survey_bank.status='0'";
												//echo "<br>";
											...
											?>
													...
													<li>
														<a href="survey_bank.php?category_id=<?= $datacategory['category_id'] ?>&tool_study_year=&client_name=&searchdata="><span <?php if ($datacategory['category_id'] == $_REQUEST['category_id']) {
																																														echo  "class='btn btn-success'";
																																													} ?>><?php echo $datacategory['category_name']; ?></span> <span class="badg"><?php echo $row['total']; ?></span></a>
													</li>
											<?php }
											} ?>
										</ul>

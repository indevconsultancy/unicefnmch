					...
					foreach ($languagegroups as $lk => $lgv) {
						...
						if ($lvgid == $languageid) {
							...
							foreach ($questions as $question) {
								...
								$rosterGroupsQuestions = [];
								if($repeated>0){
									$position_id = $rosterGroupsQuestions[$repeated]->position_id;
									$repeatedScreenGroups[] = $group_name = $rosterGroupsQuestions[$repeated]->group_name;
								...
								<?php 
									...
									<fieldset id="ss<?= $i; ?>">
										...
										<!--khushboo <input type="hidden" value="<?= $question_id; ?>" name="questions[<?= $field_name; ?>]" /> -->
											
										<input type="<?=$question_input_type;?>"  class="form-control" id="<?= $field_name; ?>" name="<?= $field_name; ?>"  maxlength="<?=$max_limit;?>" oninput="javascript: if (this.value.length > this.maxLength) this.value = this.value.slice(0, this.maxLength);" placeholder="<?= $question_name; ?>" required />
											...
											<fieldset id="ss<?= $i; ?>">
												...
												<?php
													...
													foreach ($question_options as $questionoption) {
														...
														// echo $questionoption->option_type;
													?>
														<li onclick="setRadioValue(<?= $questionoption->option_id; ?>,'<?= $field_name; ?>','<?= $field_name.$questionoption->option_id; ?>')"  ><i class="fa fa-circle-o <?= $field_name; ?>" id="<?= $field_name.$questionoption->option_id; ?>"></i> <?= $questionoption->option_value; ?> </li>
														<!--<input type="radio" name="<?= $field_name.$i; ?>"  value="<?= $questionoption->option_id; ?>" onclick="setRadioValue(this.value,<?= $field_name; ?>)" /> <?= $questionoption->option_value; ?> <br>-->
													<?php
													}
												?>
												</ul>
									...
									if(count($singleScreenQuestions)>0){ $i++; ?>
										...
										<?php	
											...
											<?php if (strtolower($question_input_type1) == 'note') { ?>
												<!-- <h2 class="fs-title">Question <?= $i; ?> -><?= $field_name1; ?></h2> -->
								...
								<!---SINGLE SCREEN QUESTIONS FORMS END ---->
					<?php
							}
						}
					}
...
<script>
	...
	$(document).ready(function(){
		...
		$("#startinterview").click(function(){
			...
			var redirectURL = 'https://mquad.org/mis/skip_userform.php?survey_id=' + surId + '&langId=' + langId;
			//alert(redirectURL);
			 if(langId!=""){
	...
	$(".next").click(function() {
		...
		// var getRelevant = "#relevant"+$(this).val();
		// var relevantVal = $(getRelevant).val();
		//console.log($(getRelevant).val());
		//console.log($(enterdata).val());
		// var c = splitMulti('a=b,c:d', ['=', ',', ':']);
		// console.log(c);
			...
			$.ajax({
				...
				success:function(res){
					...
					if(statusResponse.success=="1"){
						window.location.href='https://mquad.org/mis/mis/welcome_skip.php';
					}else{

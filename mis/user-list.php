...
?>
<?php
$page = sanitizeInput($_GET['page'], $conn);
$pages = '';
if ($page != '') {
...
if (isset($_REQUEST['search'])) {
	if (isset($_REQUEST['name']) && ($_REQUEST['name'] != "")) {
		$_REQUEST['name']=sanitizeInput($_REQUEST['name'], $conn);
		$qry .= " AND users.name like '%" . sanitizeInput($_REQUEST['name'], $conn). "%' ";
	}
	if (isset($_REQUEST['user_type_id']) && $_REQUEST['user_type_id'] != '') {
		$_REQUEST['user_type_id']=sanitizeInput($_REQUEST['user_type_id'], $conn);
		$qry .= " AND users.role_id='" . $_REQUEST['user_type_id'] . "'";
	}
	if (isset($_REQUEST['email']) && $_REQUEST['email'] != '') {
		$_REQUEST['email']=sanitizeInput($_REQUEST['email'], $conn);
...
$page_url = isset($_GET['name']) ? $page_url . "name=" . sanitizeInput($_GET['name'], $conn) : $page_url;
$page_url = isset($_GET['user_type_id']) ? $page_url . "user_type_id=" . sanitizeInput($_GET['user_type_id'], $conn) : $page_url;
$page_url = isset($_GET['email']) ? $page_url . "&email=" . sanitizeInput($_GET['email'], $conn): $page_url;
$page_url = isset($_GET['status']) ? $page_url . "&status=" . sanitizeInput($_GET['status'], $conn) : $page_url;
$page_url = isset($_GET['search']) ? $page_url . "&search=" . sanitizeInput($_GET['search'], $conn) : $page_url;
...
if (isset($_POST['setUserPermission'])) {
	$user_id = sanitizeInput($_POST['user_id'], $conn);
	$survey_id = sanitizeInput($_POST['survey_id'], $conn);
	$permissions = sanitizeInput($_POST['permissions'], $conn); //arr
...
<link rel="stylesheet" href="<?=base_url();?>css/sumoselect.css">
<link href="<?=base_url();?>css/select2.min.css" rel="stylesheet" />
<link href="<?=base_url();?>/css/bootstrap-toggle.min.css" rel="stylesheet">
...
<section id="main-content">
	<section class="wrapper">
			...
			<div class="col-lg-12">
				<nav aria-label="breadcrumb">
					<ol class="breadcrumb">
						...
						<li class="breadcrumb-item" aria-current="page"><i class="fa fa-list"></i>List Users</li>
					</ol>
				</nav>
			</div>
		</div>
			...
			<div class="col-sm-12">
				<div class="container-fluid1">
					<form class="form-inline" method="get" role="form">
						<div class="row g-2 filter_css clearfix mb-3">
				...
				<section class="panel p-1">
					<header class="table-title d-flex justify-content-between align-items-center">Total Users: <?= $total_record ?> || Active: <?= $userdata['total_active_user'] ?> || Inactive: <?= $userdata['total_inactive_user'] ?>
						...
						<a href="user_bulk_upload.php" class="btn btn-success btn-sm " >+ Upload User</a>
						<!--<a href="app_version.php" class="btn btn-primary btn-sm pull-right" style="float:right; margin:2px;">App Version</a>-->

					</header>

					<div class="table-responsive" style="min-height: 300px !important; overflow-y: auto">
						<table class="table table-hover">
							...
							<tbody>
								...
								?>
										<tr id="usid-<?= $user['user_id']; ?>">
											...
											<td class="text-center">
												<a href="view-user.php?id=<?= $user['user_id'] ?>">
													<?= getcount($conn, 'assign_survey', 'survey_id', 'status', '0', 'user_id', $user['user_id']); ?>
												</a>
											</td>
											<td><?= date('d-M-Y', strtotime($user['created_at'])); ?></td>
											<td>
												<?php if ($user['user_status'] == 1) { ?>
													<a href="javascript:void(0);" data-id="<?= $user['user_id']; ?>" class="btn btn-danger btn-sm activeUser"><i class="fa fa-times-circle" style="font-size:15px"></i></a>
												<?php } else { ?>
													<a href="javascript:void(0);" data-id="<?= $user['user_id']; ?>" class="btn btn-success btn-sm inactiveUser"><i class="fa fa-check-circle" aria-hidden="true" style="font-size:15px"></i></a>
												<?php } ?>
											</td>
											<td>
												<div class="dropdown">
													<button class="btn btn-primary btn-sm dropdown-toggle" type="button" id="export-btn" data-bs-toggle="dropdown" aria-expanded="false">
														Action
													</button>
													<ul class="dropdown-menu" aria-labelledby="export-btn">
														<li><a class="dropdown-item" href="view-user.php?id=<?= $user['user_id']; ?>">View</a></li>
														<li><a class="dropdown-item" href="edit-user.php?id=<?= $user['user_id']; ?>&page=<?= $pages; ?>">Edit</a></li>
														<li><a class="dropdown-item resetuserid" href="javascript:void(0);" data-id="<?= $user['user_id']; ?>">Reset</a></li>
														<?php if ($_SESSION['role_id'] == 1) { ?>
															<li><a class="dropdown-item delUser" href="javascript:void(0);" data-id="<?= $user['user_id']; ?>">Delete</a></li>
														<?php } ?>
													</ul>
												</div>
								...
								<?php
									...
									echo '<tr><td colspan="8" class="text-center" style="font-size: 25px;">Records Not Found !!</td></tr>';
								}
								?>
							</tbody>
						</table>
		...
		<div class="d-flex d-title-box-block align-items-center justify-content-between footer-paging mb-4">
			<div class="col-md-10">

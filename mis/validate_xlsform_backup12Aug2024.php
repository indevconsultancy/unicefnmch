				...
				<section class="panel">
					...
					<div class="panel-body">
						...
						if (isset($_POST['VerifySurvey'])) {
							...
							$typeArr = ['number','select_one','select_multiple','text','note','date','begin_group','end_group','begin_repeat','end_repeat','calculate','picture','audio','video','gps-button'];
							...
							if ($file_check == 1) {
								...
								if (!in_array($excelFormat1, $fixedHeaders)) {
									...
									if ($sheetSno == "0") {
										...
										for ($row = 2; $row <= $highestRow; $row++) {
											...
											foreach ($rowData as $key => $rowvalue) {
												...
												//$allQuesType = array($type);
												 // echo "</br>";
												// print_r($type);
												// if (!in_array($allQuesType, $fixedTypes)) {
													// $error_arr[] =  "Type ".$type." does not exist: ". $sn;
												// }
									...
									if ($sheetSno == "1") {
										...
										for ($row = 2; $row <= $highestRow; $row++) {
											...
											foreach ($rowData as $key => $rowvalue) {
												...
												if ($option_value == "") {
													$error_arr[] = "Option value missing in choices sheet, line No.: " . $ddkey;
												}

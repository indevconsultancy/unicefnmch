...
foreach ($allGroupsscreen as $gkey => $allGroupsscreen1) {
	...
	if ($group_type == 'screen') {
		...
		$scrnQues[$anon_field[$gkey]] = $gname;
	} else {
...
foreach ($survey_data as $seqKey => $surveydata1) {
	...
	//START GROUP QUESTION DISPLAY
	if (array_key_exists($surveydata1->field_name, $scrnQues)) {
		$groupName = $scrnQues[$surveydata1->field_name];
		$group_data = $full_json->$groupName;


		if (!empty($group_data)) {
			foreach ($group_data[0] as $groupdata) {
				// foreach ($groupArray as $subArray) {
				// 	foreach ($subArray as $groupdata) {
				//echo $groupdata;
	...
	//END GROUP QUESTION DISPLAY
	//START ROSTER GROUP QUESTION DISPLAY
	if (in_array($surveydata1->field_name, $grpkey)) {
		...
		$jsn_data[$groupName] = $grpdata;
	}//END ROSTER GROUP QUESTION DISPLAY
}
...
if (isset($_POST['reviewed_data'])) {
	...
	$updatedful_json =  $full_json_data;
			...
			<div class="col-lg-12">
				<nav aria-label="breadcrumb">
					<ol class="breadcrumb">
						...
						<li class="breadcrumb-item" aria-current="page"><i class="fa fa-bars"></i>Record Data</li>
					</ol>
				</nav>
			</div>
		</div>
							...
							<div class="panel-body">
								<div class="table-responsive">
									<div class="mb-4"> <?= $details_data->survey_id; ?>
										...
										<span class="label label-danger pull-right" style="margin-bottom: 2px;">Error Count: <?= $error_record ?></span>
									</div>
									<table class="table">
															...
															<tr>
																...
																<td><span class="repated-ques"><?php if (empty($jsndata2) && $jsndata2 != "0") {
																		...
																		<?php if ($repeatinfo != '') {
																			...
																			<a href="javascript:" data-bs-placement="left" data-bs-toggle="tooltip" class="btn-sm btn-secondary tooltips pull-right" title=" <?= $infoRepdata; ?> " data-bs-html="true"><i class="fa fa-info-circle" aria-hidden="true"></i></a>
																		<?php }  ?>
																	</span>
																</td>
													...
													<tr>
														...
														<td>
															...
															<?php if ($info != '') {
																...
																$info_data .= $keystroke; ?>
																<a href="javascript:" data-bs-placement="left" data-bs-toggle="tooltip" class="btn-sm btn-secondary tooltips pull-right" title=" <?= $info_data; ?> " data-bs-html="true"><i class="fa fa-info-circle" aria-hidden="true"></i></a>
															<?php } ?>

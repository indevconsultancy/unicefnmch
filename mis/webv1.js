...
$(document).ready(function(){
	...
	$(document).on("click", ".addsavequesbtn", function () {	
		$(this).closest('.add-fields').find('.addsavequesbtn').addClass('tempsavequesbtn');
	});
	
	
	$(document).on("click", ".tempsavequesbtn", function(e){
		...
		$(".ques").each(function(){
			...
			if(typeof ques_type === "undefined"){
				ques_type = $(this).find(".q_type").val();
			} 
		...
		/*	if(ques_name==''){
			...
			// console.log(incorrectGroup);
				/* if(typeof ques_type === "undefined"){
					var incorrectGroup = 'yes';
				}
				
				if(typeof incorrectGroup !== "undefined"){
					incorrectGroup = undefined;
				} */
				...
				question['required'] = ques_required === '' ? undefined : ques_required;
				question['paradata'] = checkedParadataValues === '' ? undefined : checkedParadataValues;
				question['appearance'] = ques_appearance;
				question['read_only'] = ques_read_only === '' ? undefined : ques_read_only;
				question['unique_id'] = ques_unique_id === '' ? undefined : ques_unique_id;
				...
				question['deidentify'] = ques_deidentify === '' ? undefined : ques_deidentify;
				question['preserve'] = ques_preserve === '' ? undefined : ques_preserve;
				question['parameters'] = ques_parameters === '' ? undefined : ques_parameters;
		...
		if(questions.length>0){
			...
			if(AllErrors==false){
				// console.log(questions);
				$.ajax({
					url:"https://unicef.indevconsultancy.in/mis/webajax.php",
					type:"post",
					...
					success:function(res){
						...
						if(ress.status=="1"){
							customAlert('Questions saved successfully',icon='success');
						}
			...
			let link = 'https://unicef.indevconsultancy.in/mis/add-question-web-v1.php?survey_id='+surveyId+'&res=warning';
			window.location.href=link;
		}
	});
	
	// $(".savequesbtn").on("click", function(e){
	$(document).on("click", ".savequesbtn", function(e){
		...
		$(".ques").each(function(){
			...
			$(this).next().children().find(".choice-filter-val").each(function() {
				if ($(this).val().trim() === '') {
						hasError = true;
						$(this).addClass('error');
				}
			...
			if(ques_type !== "end_group"){
				...
				// console.log('this is not an end group');
					if(hasError){
						// $(this).addClass(' error');
						// console.log('1');
						$(this).parent().parent().find('.form-error').show();
						$(this).parent().parent().find('.form-success-notice').hide();
						// $(this).closest('.dragto').find('.form-error').show();
						// $(this).closest('.dragto').find('.form-success-notice').hide();
			...
			if(AllErrors==false){
				...
				$.ajax({
					...
					success:function(res){
						...
						let ress = JSON.parse(res);
						if(ress.status=="1"){
							let link = 'https://unicef.indevconsultancy.in/mis/add-question-web-v1.php?survey_id='+surveyId+'&res=success';
							window.location.href=link;
			...
			success:function(res){
				...
				$.ajax({
					...
					success:function(res){
						...
						let ress = JSON.parse(res);
						if(ress.status=="1"){
							let link = 'https://unicef.indevconsultancy.in/mis/survey-list.php?res=success';
							window.location.href=link;
						}

...
// && $_REQUEST['process']=='publishWebFormUpload'
		...
		if($publishStatus == 0){
			...
			//$file_name =''$filename.'.'.$unique_id.'.xls';
			$file_name = "web-form-".$unique_id.".".$ext;
			 if($survey_id!=''){
				$insertForm = mysqli_query($conn,"UPDATE survey SET questinnour_file='".$file_name."' where id='".$survey_id."'");
				
			} 
			
			$fileURL = 'https://mquad.org/mis/uploaded_questionnaire/'.$clientId.'/'.$questinnour_file;
			// Check if the URL is accessible
			if (filter_var($fileURL, FILTER_VALIDATE_URL)) {
				
				// Download the Excel file
				//$tempFile = tempnam(sys_get_temp_dir(), 'excel_');
				$tempFile = tempnam(sys_get_temp_dir(),$questinnour_file);

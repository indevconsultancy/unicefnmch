    ...
    <section class="wrapper">
        ...
        <?php
			...
			if(isset($_POST['add_survey']) && $_SESSION['ISMEMORYFULL']==false && !empty($_POST['survey_name'])){
				...
				$survey_Name_field = $_POST['survey_name'];
				...
				if($surveyname==$survey_Name_field){
					...
					// echo "<script>alert('Form name already exit this project');</script>";
					// echo "<script window.location.href='add-survey.php'></script>";
				}else{
						...
						foreach ($objPHPExcel->getWorksheetIterator() as $worksheet) {
							...
							if($sheetSno=='0'){
								...
								for ($row = 2; $row <= $highestRow; $row++) {
									...
									if($row==2){
										$fixedHeaders = ['type','choice_relation','name','label','dictionary_label','lookups','media_file','preserve','unique_id','default_response','hint','limit','constraint','constraint_message','required','paradata','appearance','choice_filter','relevant','parameters','repeat_count','read_only','calculation'];
									...
									foreach ($rowData as $key => $rowvalue) {
										...
										if ($type != ''){
											...
											if($type=='begin_group' && $relevant!="" && $appearance!="fieldlist"){
											...
											if($language_id=="1"){
												...
												if($type=="begin_repeat"){ 
													...
													if($parameters=="multiroster"){ $field_nameBgn=$repeat_count; }else{ $field_nameBgn=$field_name; } 
													...
													$update = mysqli_query($conn,"UPDATE questions SET repeated='start',repeat_count='".$field_nameBgn."', group_id='".$insert_group_id."', parent_relevent='".$prelv."', roster_relevent='".$relevant."' WHERE question_id='".$qid_insert."' ");
													
													//$txt = "Raju";
													$reptno=(int)$repeat_count;
													if(is_int($reptno) && $reptno>0){
														$update = mysqli_query($conn,"UPDATE questions_language SET repeated='".$insert_group_id."',repeat_count='".$repeat_count."', parent_relevent='".$prelv."', roster_relevent='".$relevant."' WHERE question_id='".$qid_insert."' ");
													}else{
														$update = mysqli_query($conn,"UPDATE questions_language SET repeated='".$insert_group_id."',repeat_count='".$field_nameBgn."', parent_relevent='".$prelv."', roster_relevent='".$relevant."' WHERE question_id='".$qid_insert."' ");
													}
												}
												if($type=="end_repeat"){
													$qid_insert="";
                                        ...
                                        <div class="form">
                                            <form class="form-validate form-horizontal" method="post" enctype="multipart/form-data">
                                                ...
                                                <div class="form-group " style="display: none;" id="upload_structure">
                                                    ...
                                                    <div class="col-lg-10">
                                                        <input class="form-control tooltips" name="structure" id="structure" accept=".xlsx,.xls," data-placement="top" data-toggle="tooltip"  minlength="5" type="file" />
												   </div>
                                                </div>
                                                <div class="form-group">
                                                    <div class="col-lg-offset-2 col-lg-10">
														<button class="btn btn-secondary" type="submit" name="add_survey" id="add_survey" <?=$_SESSION['BTNBLOCK'];?> >Submit</button>
...
$(document).ready(function(){
	$('#survey_name_txt').keyup(function(){
		...
		$.ajax({
			url:"add-survey-ssajax.php",
			method:"POST",
			data:{survey_name_txt:survey_name_txt},
			dataType:"text",

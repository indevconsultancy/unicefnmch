                ...
                <div class="col-lg-12">
                    <ol class="breadcrumb">
                       <!-- <li><i class="fa fa-home"></i><a href="dashboard.php">Home</a></li> -->
                        <li><i class="fa fa-user" aria-hidden="true"></i>Client Management</li>
                       <li><i class="fa fa-list"></i>List Clients</li>
                    </ol>
                </div>
            </div>
            <!-- page start-->
            <div class="row">
                <div class="col-sm-12">                  
							...
							<div class="row filter_css clearfix">
                                <div class="col-lg-5" style="margin-bottom: 1rem!important;margin-top: -1rem!important;">
                                    <select class="form-control" name="id" id="id">
										...
										<option value="<?php echo $type['id']?>" <?php if($type['id']==$_REQUEST['id']) { echo "selected";}?>><?php echo $type['name']?></option>
										<?php	
										}
                        ...
                        <table class="table table-striped">
                            ...
                            <tbody>
                            <?php
							$_SESSION['query']="select clients.id,clients.name,clients.email,roles.name as role_type_name,clients.status from clients INNER JOIN roles on clients.role_id=roles.id where clients.role_id!='1' AND clients.del_action='N' order by clients.id DESC";
							$sql="select clients.id,clients.name,clients.email,roles.name as role_type_name,clients.status from clients INNER JOIN roles on clients.role_id=roles.id where clients.role_id!='1' AND clients.del_action='N' $qry order by clients.id DESC limit $page,$per_page";

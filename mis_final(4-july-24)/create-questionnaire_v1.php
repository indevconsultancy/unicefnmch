<?php
	...
	function createExcelSheet($jsondata){
        ...
        foreach ($cols as $headKey => $colheder) {
			$activeSheet->setCellValue($setValuePosiotion[$headKey] . 1, $colheder);
		}
		...
		foreach ($colChoices as $choiceKey=>$colChoice) {
			//echo $choiceKey;
			$activeSheet1->setCellValue($setValuePosiotion[$choiceKey] . 1, $colChoice);
		}
		// prepare the rest

...
// 	$keywords_id = array();
// 	foreach ($keywords_array as $key => $keywords_array) {
// 		$check_keyword_query = mysqli_query($conn, "SELECT keywords_id,keyword_name FROM `keywords` where keyword_name='" . $keywords_array . "'");
// 		$num_rows_check = mysqli_num_rows($check_keyword_query);
// 		$row_keyword = mysqli_fetch_array($check_keyword_query);
// 		$keyword_id1 = $row_keyword['keywords_id'];
// 		if ($num_rows_check > 0) {
// 			$keywords_id[] = $keyword_id1;
// 		} else {
// 			$inskeywords = "INSERT INTO keywords (keyword_name) VALUES ('$keywords_array')";
// 			mysqli_query($conn, $inskeywords);
// 			$keywords_id[] = mysqli_insert_id($conn);
// 		}
// 	}
// 	$keywords_id_str = implode(",", $keywords_id);
// 	$authors_id = array();
// 	foreach ($authors_array as $key => $authors_arrays) {
// 		$check_author_query = mysqli_query($conn, "SELECT authors_id,author_name FROM `authors` where author_name='" . $authors_arrays . "'");
// 		$num_rows_check = mysqli_num_rows($check_author_query);
// 		$row_author = mysqli_fetch_array($check_author_query);
// 		$authors_id1 = $row_author['authors_id'];
// 		if ($num_rows_check > 0) {
// 			$authors_id[] = $authors_id1;
// 		} else {
// 			$insauthorwords = "INSERT INTO authors (author_name) VALUES ('$authors_arrays')";
// 			mysqli_query($conn, $insauthorwords);
// 			$authors_id[] = mysqli_insert_id($conn);
// 		}
// 	}
// 	$author = implode(",", $authors_id);
// 	$institution_id = array();
// 	foreach ($institution_array as $key => $institution_arrays) {
// 		$check_institution_query = mysqli_query($conn, "SELECT institution_id,institution_name  FROM `institution` where institution_name='" . $institution_arrays . "'");
// 		$num_rows_check = mysqli_num_rows($check_institution_query);
// 		$row_institution = mysqli_fetch_array($check_institution_query);
// 		$nstitution_id1 = $row_institution['institution_id'];
// 		if ($num_rows_check > 0) {
// 			$institution_id[] = $nstitution_id1;
// 		} else {
// 			$insauthorwords = "INSERT INTO institution (institution_name ) VALUES ('$institution_arrays')";
// 			mysqli_query($conn, $insauthorwords);
...
// 	$insert_query = "INSERT INTO project_datasets (title, description, type_of_study_id, type_of_study_other, thematic_area_id, keywords_id, authors_id, institution_id, contact_person_name, contact_person_email, collaboration_id, user_id, year, related_publication, data_type, country_id, state_id, project_id) VALUES ('$title', '$description', '$type_of_study','$other_specify', '$thematic_area_str', '$keywords_id_str', '$author', '$institution', '$contact_person_name', '$contact_person_email', '$collaboration_id', '$user_id', '$Years','$related_publication','$data_type','$country_id','$stateId','$project_id')";
...
if (isset($_POST['submit'])) {
	...
	$description = mysqli_real_escape_string($conn, $_POST['description']);
	$type_of_study = $_POST['study_type_id'];
	$other_specify = mysqli_real_escape_string($conn, $_POST['other_specify']);
	$thematic_id = $_POST['category_id'];
	$keywords_array = $_POST['keywords'];
	$institution_array = $_POST['institution_company'];
	$authors_array = $_POST['author'];
	...
	$insert_query = "INSERT INTO project_datasets (title, description, type_of_study_id, type_of_study_other, thematic_area_id, keywords_id, authors_id, institution_id, contact_person_name, contact_person_email, collaboration_id, user_id, related_publication, data_type, country_id, state_id, project_id,use_of_term,from_date,to_date) VALUES ('$title', '$description', '$type_of_study', '$other_specify', '$thematic_area_str', '$keywords_id_str', '$author', '$institution', '$contact_person_name', '$contact_person_email', '$collaboration_id', '$user_id', '$related_publication', '$data_type', '$country_id', '$stateId', '$project_id','$use_of_term','$from_date','$to_date ')";
	$result = mysqli_query($conn, $insert_query);
	$lastId = mysqli_insert_id($conn);
	$folderName = "D" . $lastId;
	$location = "../upload_data_file/dataset/" . $folderName . "/";
		...
		<div class="row">
			<div class="col-lg-12">
				...
				<ol class="breadcrumb">
					...
					<li><i class="fa fa-plus"></i>Add Dataset(s)</li>
				</ol>
			</div>
		</div>
		<!-- page start-->
						...
						<div class="form">
							<form class="form-validate form-horizontal" id="myForm" method="post" enctype="multipart/form-data">
								...
								<div class="form-group">
									...
									<div class="col-lg-10">
										<div class="form-check">
											<select name="category_id[]" class="form-control select2" id="category_id" multiple required data-placeholder="Select Thematic Area(s)">
												<?php
								...
								<div class="form-group ">
									...
									<div class="col-lg-10">
										<div class="form-check">
											<select class="form-control" multiple="multiple" required id="keywords" data-tags="true" name="keywords[]">
											</select>
										</div>
									</div>
								</div>
								<div class="form-group">
									...
									<div class="col-lg-10">
										<div class="form-check">
											<select class="form-control" multiple="multiple" id="institution_company" required="" data-tags="true" name="institution_company[]">
											</select>
										</div>
									</div>
								</div>
								...
								<div class="form-group">
									...
									<select class="form-control form-select" name="country_id" onchange="getState(this.value)">
								...
								<div class="form-group ">
									...
									<div class="col-lg-10">
										<div class="form-check">
											<select class="form-control" multiple="multiple" required id="author" data-tags="true" name="author[]">
											</select>
										</div>
									</div>
								</div>
								...
								<div>
									<label class="control-label col-lg-2" for="use_of_term">Terms of Data Use</label>
									<div class="col-lg-10">

										<textarea cols="30" rows="5" class="summernote" name="use_of_term" id="use_of_term">
										<p>Submission of the Application for Data Use is required to access the data from this study.</p>
										<br>
										...
										<br>
										<p>Special Permissions: To access data from this study, submit the Application for Data Use.</p>
									        </textarea>
									</div>
								</div>

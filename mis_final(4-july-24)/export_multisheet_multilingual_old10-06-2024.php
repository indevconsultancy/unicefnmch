...
//$exp_repeated_columns_old=array();
...
//echo "SELECT question_id as qid,option_sequence, option_name,(SELECT DISTINCT(questions_language.field_name) FROM questions_language WHERE questions_language.question_id=qid) AS field_name FROM options_language  WHERE survey_id='".$survey_id_exported."' AND language_id='".$language_id."' ";
$allOptionss = mysqli_query($conn,"SELECT question_id as qid,option_sequence, option_name,(SELECT DISTINCT(questions_language.field_name) FROM questions_language WHERE questions_language.question_id=qid) AS field_name FROM options_language  WHERE survey_id='".$survey_id_exported."' AND language_id='".$language_id."' ");
...
//-----------getting query from survey------------------//

$get_sdmon = mysqli_query($conn,"SELECT survey_data_monitoring_id,survey_status,termination_reason,survey_id,users.name as full_name, survey_name, survey_data_json_coded, full_json  FROM survey_data_monitoring left join users on users.user_id=survey_data_monitoring.user_id WHERE survey_name_id = '".$survey_id_exported."' and survey_status!=7 order by survey_data_monitoring_id asc");
$counter=2;
while($sdmon = mysqli_fetch_object($get_sdmon)){
        ...
        foreach($exp_columns as $k=>$exp_column){
            ...
            $export_main_value[]=$jsondata[$exp_column]; //str_replace(",","~",$jsondata[$exp_column])?:"NA";
			 $mon_data_value = $jsondata[$exp_column];
			 $mon_data_value = str_replace(array("-/", "/"), array("-", "-"), $mon_data_value);
    
			//print($mon_data_value);
...
$get_expt_fields_old = mysqli_query($conn,"SELECT field_lable FROM report_format WHERE survey_id='".$survey_id_exported."' AND group_id!='0' AND title!='' $accessLabels ORDER BY seq_no ASC");
while($expt_fields_old = mysqli_fetch_object($get_expt_fields_old)){
    ...
    $exp_repeated_columns_old[] = $expt_fields_old->field_lable;
}
...
$get_repeated_sdmon = mysqli_query($conn,"SELECT survey_data_monitoring_id,survey_id, survey_data_json, full_json  FROM survey_data_monitoring WHERE survey_name_id = '".$survey_id_exported."' order by survey_data_monitoring_id asc");
    ...
    foreach($fulljsondata as $fulljsonKey=>$repeated){
        ...
        if(is_array($repeated)){
            if($fulljsonKey!="survey_data" && $fulljsonKey!="hint_record" && $fulljsonKey!="error_record"){
				...
				$get_expt_fields = mysqli_query($conn,"SELECT id,title,field_lable,msci_report FROM report_format WHERE survey_id='".$survey_id_exported."' AND group_id='".$group_id."' AND title!='' $accessLabels ORDER BY seq_no ASC");
				while($expt_fields = mysqli_fetch_object($get_expt_fields)){
                ...
                foreach($repeated as $rptkey=>$repeatedq){
                    ...
                    foreach($repeatedq as $repeatedqs){
						...
						if($option_id!=""){
							...
							$getoption = mysqli_query($conn,"SELECT option_name FROM options_language WHERE question_id='".$question_id."' AND option_sequence='".$option_id."' and language_id='".$language_id."' ");
...
/* 
$file_name = "Excel-".str_replace(" ","_",$survey_name).".xlsx";
...
// $path = "https://mquad.org/mis/uploaded_questionnaire/abc.xlsx";
// $resArr = array('status'=>200,'path'=>$path,'fname'=>$file_name,'message'=>'Success');

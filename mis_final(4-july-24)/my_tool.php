    ...
    <section class="wrapper">
        <div class="row">
            <div class="col-lg-12">
                <ol class="breadcrumb">
                    ...
                    <li><i class="fa fa-list"></i>My Tools </li>
                </ol>
            </div>
        </div>
        <!-- page start-->
                    ...
                    <form method="GET" style="padding: 10px 0px;">
                        <div class="row">
                            <div class="form-group col-md-4" style="margin-bottom: 1rem!important; margin-top: -1rem!important;">
                                <input type="text" class="form-control" name="survey_title" id="survey_title" value="<?= @$_REQUEST['survey_title'] ?>" placeholder="Name	" title="Name" data-toggle="tooltip" data-placement="top">
                            </div>
                            <div class="form-group col-md-2" style="margin-bottom: 1rem!important; margin-top: -1rem!important;">
                                <input class="form-control" id="from_datepicker" placeholder="From Date" value="<?= @$_REQUEST['from_date'] ?>" name="from_date" type="text" />

                            </div>
                            <div class="form-group col-md-2" style="margin-bottom: 1rem!important; margin-top: -1rem!important;">
                                <input class="form-control" id="to_datepicker" placeholder="To Date" value="<?= @$_REQUEST['to_date'] ?>" name="to_date" type="text" />
                            </div>
        ...
        <section class="panel">
            <header class="panel-heading d-flex justify-content-between align-items-center">
                <div>Number of My Tool(s): <?= $total_record ?></div>
                                ...
                                <tr>
                                    ...
                                    <?php if ($_SESSION['role_id'] == '1') { ?>
                                        <td>
                                            <form method="post" action="">
                                                <?php
                                                if ($gettool['tool_archive_status'] == '0') { ?>
                                                    <button class="btn btn-warning btn-sm" data-target="#Modalassignque<?= $gettool['id'] ?>" onclick="assignque(<?= $gettool['id']; ?>)" ; type="button" data-toggle="modal" data-keyboard="false" data-whatever="@fat">
                                                        Pending</button>
                                                <?php
                                                } else if ($gettool['tool_archive_status'] == '1') {
                                                ?>
                                                    <button class="btn btn-success btn-sm" data-target="#Modalassignque<?= $gettool['id'] ?>" onclick="assignque(<?= $gettool['id']; ?>)" ; type="button" data-toggle="modal" data-keyboard="false" data-whatever="@fat">
                                                        Approved</button>

                                                <?php
                                                } else if ($gettool['tool_archive_status'] == '2') {
                                                ?>
                                                    <button class="btn btn-danger btn-sm " data-target="#Modalassignque<?= $gettool['id'] ?>" onclick="assignque(<?= $gettool['id']; ?>)" ; type="button" data-toggle="modal" data-keyboard="false" data-whatever="@fat">
                                                        Rejected</button>
                                                <?php
                                                }
                                    ...
                                    <td>
                                        ...
                                        if ($_SESSION['user_id'] == $gettool['user_id']) { ?>
                                            <a href="edit-survey-bank.php?eid=<?= $gettool['id'] ?>" class="btn-sm btn-primary" title="Edit  tool"><i class="fa fa-pencil-square-o"></i></a>
                                        <?php
                                        }
                                        ?>
                                        <?php if ($_SESSION['user_id'] == $gettool['user_id'] || $_SESSION['role_id'] == '1') { ?>
                                            <a href="javascript:void(0);" data-id="<?= $gettool['id']; ?>" class="btn-sm btn-primary delQuestions "><i class="fa fa-trash"></i></a>
                                        <?php } else { ?>

                                        <?php } ?>
                                    </td>
                                    <div class="modal fade assignque_model" id="Modalassignque<?= $gettool['id'] ?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">

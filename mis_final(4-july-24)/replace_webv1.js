...
$(document).ready(function(){
	...
	$(document).on('change',".languageSelect",function() {
		...
		let constraint = $this.next().next().next().attr('placeholder','Constraint ::' + languageSelect + '');
	...
	$(document).on('change',".languageSelectOne",function() {
		...
		let constraint = $this.next().next().next().next().attr('placeholder','Constraint ::' + languageSelectOne + '');
	...
	$("#savequesbtn").on("click", function(e){
			...
			if(ques_type != 'end_group'){
				if(ques_type != 'end_repeat'){
					...
					if(typeof ques_name === "undefined") {
						...
						//$(this).next().find(".groups-header").append(errorDiv);
						hasError=true;
					}
		...
		if(questions.length>0){
			...
			if(AllErrors==false){
				// console.log('qwerty');
				$.ajax({
					url:"https://mquad.org/mis/replace_webajax.php",
					type:"post",
					// cache : false,
					// processData: false,
					// contentType: false,
					...
					success:function(res){
						...
						if(ress.status=="1"){
							customAlert('Updated Successfully, Please Re-publish the Form',icon='success');
						}
					}
				});
			}
	...
	$("#publishwebform").on("click", function(e){
		...
		$.ajax({
			...
			success:function(res){
				...
				$.ajax({
					url: "update_replace_questionnaire_ajax.php?survey_id="+sid,
					type:"GET",
					data:{},
					success:function(res){
						// console.log(res);
						let ress = JSON.parse(res);
						if(ress.status=="1"){
							customAlert('Form Updated Successfully, Please Re-publish the form',icon='success');
						}else{

...
if (isset($_POST["submit"])) {
    ...
    if (!empty($_FILES["userfile"]["name"])) {
		...
		$bucket = 'mquaddata';
        $s3Key = 'media_lookups/'.$clientID.'/'.$survey_id.'/' . $file_name;
        $destination = 'media_lookups/'.$clientID.'/'.$survey_id.'/' . pathinfo($file_name, PATHINFO_FILENAME);
        ...
        try {
			...
			$zipFileName = 'mquaddata/media_lookups/'.$clientID.'/'.$survey_id.'/' . $file_name;
            ...
            if ($finalstatus) {
                ...
                echo "Files extracted and uploaded successfully.";
            } else {
                echo "Something went wrong during extraction or upload.";
            }
        } catch (S3Exception $e) {
            echo $e->getMessage() . "\n";
        }

    ...
    if (!empty($_FILES['file']['name']) && in_array($_FILES['file']['type'], $csvMimes)) {
        if (is_uploaded_file($_FILES['file']['tmp_name'])) {
			...
			if ($errmsg != '') {
				...
				$message = 'Enter mobile number more than 18 digit.Please verify Row No.'.$errmsg;
				echo "<script>alert('$message');
				window.location.href='" . $pagelink . "';
				</script>";
				// $_SESSION['status_error'] = "Please enter mobile number more than 18 digit.Row No.".$errmsg;;

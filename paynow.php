...
if(isset($_REQUEST['payment_id'])){
 ...
 $varapp='';
 $sqlsubscriptionitems=mysqli_query($conn, "select pm_service_component.field_name,pm_subscription_items.componentValue from pm_service_component,pm_subscription_items where pm_service_component.id=pm_subscription_items.componentID and pm_subscription_items.SubscriptionID=$datapayment->SubscriptionID");
 while($dataSubscriptionitems=mysqli_fetch_object($sqlsubscriptionitems))
 {
	$varapp.=",";
	$varapp.=$dataSubscriptionitems->field_name."='".$dataSubscriptionitems->componentValue."'";
	
 }
$_SESSION['SUBSCRIPTIONEXPIRED']=false;
  mysqli_query($conn,"insert into pm_usersubscriptions set UserID='".$datapayment->UserID."',OrderID='".$datapayment->OrderID."',PaymentID='".$_SESSION['payment_id']."',client_id='".$datapayment->client_id."',SubscriptionID='".$datapayment->SubscriptionID."',StartDate='".$startdate."',EndDate='".$enddate."',Status='Active'$varapp");
//echo "insert into pm_usersubscriptions set UserID='".$datapayment->UserID."',OrderID='".$datapayment->OrderID."',PaymentID='".$_SESSION['payment_id']."',client_id='".$datapayment->client_id."',SubscriptionID='".$datapayment->SubscriptionID."',StartDate='".$startdate."',EndDate='".$enddate."',Status='Active' $varapp";

...
$sqlqry = mysqli_query($conn, "select md.id,md.registration_id,md.admission_weight,md.admission_length,md.admission_head_circumference,reg.sex,DATEDIFF(md.date_of_admission,reg.baby_date_of_birth) AS age_months,reg.gestational_age_LBW from monitoring_data md,registration_form reg where md.registration_id=reg.id and md.type_of_monitoring!='Mother and Newborn at MNCU' and md.admission_weight>0 and reg.gestational_age_LBW>=22 and md.type_of_monitoring='Date of Admission' and md.growth_status_fenton=0 limit 0,100");
